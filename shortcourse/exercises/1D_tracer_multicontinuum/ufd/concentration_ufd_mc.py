#Radnuclide transport with matrix diffusion, no decay or sorption
import sys
import os
try:
   ...
   sys.exit(1)
pflotran_dir = '/Users/rleone/software/pflotran'
...
files = pft.get_tec_filenames('ufd_mc',[1,3,5])
...
plt.legend(loc=1)
plt.savefig("conc_with_mc_fracture_am241.png")
plt.show()
...
plt.legend(loc=4)
plt.savefig("conc_with_mc_fracture_np237.png")
plt.show()


files = pft.get_tec_filenames('ufd_mc-sec-rank0-obs0',[1,3,5])
filenames = pft.get_full_paths(path,files)
...
plt.legend(loc=1)
plt.savefig("conc_with_mc_matrix.png")
plt.show()
