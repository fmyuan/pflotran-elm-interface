      ...
      if(option%iflowmode == TH_MODE) then
        ...
        select case(option%iflowmode)
          case(TH_MODE)
            ...
            th_parameter%alpha(icct)    = 1.d0/(9.81d0*sucsat_pf_loc(ghosted_id))
            th_parameter%ckwet(icct)    = tkwet_pf_loc(ghosted_id)*option%scale   ! W/m/K --> MW/m/K
            !(note: option%scale multiplier is done in TH.F90: setuppatch(), so it's needed here too)
            th_parameter%ckdry(icct)    = tkdry_pf_loc(ghosted_id)*option%scale   ! W/m/K --> MW/m/K
            th_parameter%ckfrozen(icct) = tkfrz_pf_loc(ghosted_id)*option%scale   ! W/m/K --> MW/m/K

            th_parameter%dencpr(icct)   = hcapvs_pf_loc(ghosted_id)*option%scale  ! J/m3-particle/K --> MJ/m3-particle/K
