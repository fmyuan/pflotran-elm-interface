...
subroutine SolverReadLinear(solver,input,option)
  ...
  !************************************************************************************
  ! following are options for the CPR preconditioner

      case('CPRT2_TYPE')
        call InputReadWord(input,option,word,PETSC_TRUE)
        call InputErrorMsg(input,option,'cprT2_type','LINEAR SOLVER')
        call StringToUpper(word)
        select case(trim(word))
          case('SAILS')
            solver%cprstash%T2_type = 'SAILS'
          case('PBJ')
            solver%cprstash%T2_type = 'PBJ'
          case('NONE')
            solver%cprstash%T2_type = 'NONE'
          case('PCASM')
            solver%cprstash%T2_type = 'PCASM'
          case('PCGASM')
            solver%cprstash%T2_type = 'PCGASM'
          case('PILUT')
            solver%cprstash%T2_type = 'PILUT'
          case('EUCLID')
            solver%cprstash%T2_type = 'EUCLID'
          case('ILU')
            ! this will cause crash if using more than 1 proc
            solver%cprstash%T2_type = 'ILU'
          case default
      ...
      case('CPRT1_TYPE')
        call InputReadWord(input,option,word,PETSC_TRUE)
        call InputErrorMsg(input,option,'cprT1_type','LINEAR SOLVER')
        call StringToUpper(word)
        select case(trim(word))
          case('RICHARDSON')
            !option%CPRT1_type = 'RICHARDSON'
            solver%cprstash%T1_type = 'RICHARDSON'
          case('FGMRES')
            !option%CPRT1_type = 'FGMRES'
            solver%cprstash%T1_type = 'FGMRES'
          case default
      ...
      case('CPR_EXTRACTION_TYPE')
        call InputReadWord(input,option,word,PETSC_TRUE)
        call InputErrorMsg(input,option,'cpr_extraction_type','LINEAR SOLVER')
        call StringToUpper(word)
        select case(trim(word))
          case('QIMPES_VARIABLE')
            solver%cprstash%T1_type = 'QIMPES_VARIABLE'
          case('QIMPES_VARIABLE_FORCE')
            solver%cprstash%T1_type = 'QIMPES_VARIABLE_FORCE'
          case default
      ...
      case('CPR_EX_OFFSET')
        call InputReadInt(input,option, solver%cprstash%exrow_offset)

      case('T2FILLIN')
        call InputReadInt(input,option, solver%cprstash%t2fillin)

      case('T2ASMOVERLAP')
        call InputReadInt(input,option, solver%cprstash%asmoverlap)

      case('T2SHIFTINBLOCKS')
        solver%cprstash%t2shiftinblocks= PETSC_TRUE

      case('ASMINPLACE')
        solver%cprstash%asmfactorinplace = PETSC_TRUE

      case('CPROUTPUTDEBUG')
        solver%cprstash%mv_output = PETSC_TRUE

      case('T2_ZEROING')
        solver%cprstash%t2_zeroing= PETSC_TRUE

      case('CPRZEROING')
        solver%cprstash%zeroing = PETSC_TRUE

      case('CPRSKIPT1')
        solver%cprstash%skip_T1= PETSC_TRUE

      case('CPRAMGREPORT')
        solver%cprstash%amg_report = PETSC_TRUE

      case('CPR_MANUAL_AMG_CONFIG')
        solver%cprstash%amg_manual = PETSC_TRUE

      case('CPRGAMG')
        solver%cprstash%useGAMG = PETSC_TRUE
  ...
  ! end of options for the CPR preconditioner
  !************************************************************************************
