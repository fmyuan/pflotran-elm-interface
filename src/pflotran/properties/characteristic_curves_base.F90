...
subroutine SFBaseVerify(this,name,option)
  ...
  if ((.not.this%analytical_derivative_available) .and. &
      (.not.option_flow%numerical_derivatives)) then
  ...
  ! --------------------
    ...
    select case (option_flow%ice_model)
      ...
      case (PAINTER_KARRA_EXPLICIT, PAINTER_KARRA_EXPLICIT_SMOOTH)
        ...
        if(option_flow%frzthw_halfwidth /= UNINITIALIZED_DOUBLE) deltaTf = option_flow%frzthw_halfwidth
        if (option_flow%ice_model==PAINTER_KARRA_EXPLICIT_SMOOTH .and. deltaTf>1.0d-50) then
