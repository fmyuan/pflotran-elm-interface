...
subroutine NitrifSetup(this,reaction,option)
  ...
  word = 'NH4+'
  this%ispec_nh4 = ReactionAuxGetPriSpecIDFromName(word,reaction, option)

  if(this%ispec_nh4 > 0) then
     word = 'NH4sorb'   ! this is the immobile species from 'reaction_sandbox_langmuir'
     this%ispec_nh4sorb = ReactionImGetSpeciesIDFromName(word, &
  ...
  word = 'NGASnitr'
  this%ispec_ngasnit = ReactionImGetSpeciesIDFromName( &
...
! g soil = m3_soil*kg_m3*1.d3                                ! unit: gSoil
  ...
  if( (option%print_file_flag) .and. &
    ...
    ' ratedt_nitri=',rate_nitri*option%tran_dt,' ratedt_nit_n2o=',rate_n2o*option%tran_dt, &
