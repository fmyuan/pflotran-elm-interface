      ...
      case('ICE_MODEL')
        ...
        select case (trim(word))
          case ('PAINTER_EXPLICIT')
            option%flow%ice_model = PAINTER_EXPLICIT
          case ('PAINTER_KARRA_IMPLICIT')
            option%flow%ice_model = PAINTER_KARRA_IMPLICIT
          case ('PAINTER_KARRA_EXPLICIT')
            option%flow%ice_model = PAINTER_KARRA_EXPLICIT
          case ('PAINTER_KARRA_EXPLICIT_NOCRYO')
            option%flow%ice_model = PAINTER_KARRA_EXPLICIT_NOCRYO
          case ('PAINTER_KARRA_EXPLICIT_SMOOTH')
            option%flow%ice_model = PAINTER_KARRA_EXPLICIT_SMOOTH
            call InputReadDouble(input,option,tempreal)
            call InputDefaultMsg(input,option,'freezing-thawing smoothing')
            if(tempreal > 1.d-10) option%flow%frzthw_halfwidth = tempreal
          case ('DALL_AMICO')
            option%flow%ice_model = DALL_AMICO
          case default
