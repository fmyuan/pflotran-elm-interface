  ...
  do
    ...
    select case(trim(keyword))
      case('GAS_COMPONENT_FORMULA_WEIGHT')
        ...
        call InputErrorMsg(input,option,keyword,error_string)
      case('NO_FRACTURE')
        zflow_use_fracture = PETSC_FALSE
      case('NO_CREEP_CLOSURE')
        zflow_use_creep_closure = PETSC_FALSE
      case('NO_GAS_GENERATION')
        zflow_use_gas_generation = PETSC_FALSE
      case('BRAGFLO_RESIDUAL_UNITS')
        zflow_use_bragflo_units = PETSC_TRUE
      case('DEBUG')
        zflow_debug = PETSC_TRUE
      case('DEBUG_GAS_GENERATION')
        zflow_debug_gas_generation = PETSC_TRUE
      case('DEBUG_FIRST_ITERATION')
        zflow_debug = PETSC_TRUE
        zflow_debug_first_iteration = PETSC_TRUE
      case('DEBUG_OSCILLATORY_BEHAVIOR')
        zflow_check_oscillatory_behavior = PETSC_TRUE
      case('DEBUG_TS_UPDATE')
        zflow_debug_ts_update = PETSC_TRUE
      case('MATCH_BRAGFLO_OUTPUT')
        zflow_match_bragflo_output = PETSC_TRUE
      case('USE_LEGACY_PERTURBATION')
        zflow_use_legacy_perturbation = PETSC_TRUE
      case('USE_BRAGFLO_CC')
        zflow_use_bragflo_cc = PETSC_TRUE
      case('RESIDUAL_TEST')
      ...
      case('ALLOW_NEGATIVE_GAS_PRESSURE')
        zflow_allow_neg_gas_pressure = PETSC_TRUE
      case('HARMONIC_PERMEABILITY_ONLY')
        zflow_use_lumped_harm_flux = PETSC_FALSE
      case('DEFAULT_ALPHA')
        zflow_default_alpha = PETSC_TRUE
      case('ALPHA_DATASET')
...
subroutine PMZFlowReadNewtonSelectCase(this,input,keyword,found, &
  ...
  select case(trim(keyword))
    ...
    case('REL_GAS_SATURATION_PERTURBATION')
      call InputReadDouble(input,option,zflow_sat_rel_pert)
      call InputErrorMsg(input,option,keyword,error_string)
    case('MIN_GAS_SATURATION_PERTURBATION')
      call InputReadDouble(input,option,zflow_sat_min_pert)
      call InputErrorMsg(input,option,keyword,error_string)
...
subroutine PMZFlowUpdateTimestep(this,dt,dt_min,dt_max,iacceleration, &
  ...
  use ZFlow_Aux_module, only : zflow_debug_ts_update
...
subroutine PMZFlowResidual(this,snes,xx,r,ierr)
  ...
  ! calculate residual
  call ZFlowResidual(snes,xx,r,this%realization,this%pmwss_ptr,ierr)
...
subroutine PMZFlowJacobian(this,snes,xx,A,B,ierr)
  ...
  call ZFlowJacobian(snes,xx,A,B,this%realization,this%pmwss_ptr,ierr)
...
subroutine PMZFlowCheckUpdatePost(this,snes,X0,dX,X1,dX_changed, &
  ...
  do local_id = 1, grid%nlmax
    ...
    pressure_index = offset + ZFLOW_LIQUID_PRESSURE_DOF
    saturation_index = offset + ZFLOW_GAS_SATURATION_DOF
  ...
  if (zflow_debug_first_iteration) stop
...
subroutine PMZFlowCheckConvergence(this,snes,it,xnorm,unorm, &
  ...
  do local_id = 1, grid%nlmax
    ...
    ghosted_id = grid%nL2G(local_id)
    if (patch%imat(ghosted_id) <= 0) cycle
    liquid_equation_index = offset + ZFLOW_LIQUID_EQUATION_INDEX
    gas_equation_index = offset + ZFLOW_GAS_EQUATION_INDEX
    ...
    if (.not.zflow_use_bragflo_units) then
    ...
    residual = bragflo_residual(ZFLOW_LIQUID_EQUATION_INDEX)
    accumulation = bragflo_accum(ZFLOW_LIQUID_EQUATION_INDEX)
    ...
    if (dabs(accumulation) > zero_accumulation) then 
      ...
      if (dabs(max_normal_res_liq_) < dabs(residual)) then
        if (zflow_match_bragflo_output) then
          if (dabs(residual) > this%liquid_residual_infinity_tol) then
    ...
    residual = bragflo_residual(ZFLOW_GAS_EQUATION_INDEX)
    accumulation = bragflo_accum(ZFLOW_GAS_EQUATION_INDEX)
    ...
    ! gas pressure
    if (zflow_auxvars(0,ghosted_id)%pres(2) < min_gas_pressure) then
      min_gas_pressure = zflow_auxvars(0,ghosted_id)%pres(2)
  ...
  if (min_gas_pressure < 0.d0 .and. .not.zflow_allow_neg_gas_pressure) then
  ...
  if (zflow_check_oscillatory_behavior) then
    do i = size(zflow_prev_liq_res_cell), 2, -1
      zflow_prev_liq_res_cell(i) = zflow_prev_liq_res_cell(i-1)
    enddo
    zflow_prev_liq_res_cell(1) = this%convergence_flags(MAX_NORMAL_RES_LIQ)
    if (zflow_prev_liq_res_cell(size(zflow_prev_liq_res_cell)) > 0) then
      cell_id_match = PETSC_TRUE
      do i = 1, size(zflow_prev_liq_res_cell)-2
        if (zflow_prev_liq_res_cell(i) /= zflow_prev_liq_res_cell(i+2)) then
          cell_id_match = PETSC_FALSE
        endif
      enddo
      if (cell_id_match) then
        zflow_print_oscillatory_behavior = PETSC_TRUE
      endif
    endif
  endif
...
!        zflow_auxvars(0,ghosted_id)%pres(1:2), &
!        zflow_auxvars(0,ghosted_id)%pres(option%capillary_pressure_id)
...
subroutine PMZFlowTimeCut(this)
  ...
  use ZFlow_Aux_module, only : zflow_prev_liq_res_cell, &
                                   zflow_print_oscillatory_behavior, &
                                   zflow_match_bragflo_output
...
subroutine PMZFlowUpdateSolution(this)
  ...
  use ZFlow_Aux_module, only : zflow_debug
  ...
  if (zflow_debug) then
    write(*,'("Final: ",10es14.6)') &
      this%realization%patch%aux%ZFlow%auxvars(0,1)%pres(1:2), &
      this%realization%patch%aux%ZFlow%auxvars(0,1)%effective_porosity, &
      this%realization%patch%aux%ZFlow%auxvars(0,1)%sat(1)
  endif
