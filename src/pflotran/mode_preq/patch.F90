...
#include "petsc/finclude/petscdmda.h"
  ...
  if (isub_condition>option%nflowdof*option%nfluid) then
    option%io_buffer='ERROR: PatchUpdateHetroCouplerAuxVars  '// &
      'isub_condition > option%nflowdof*(option%nfluid.'
    call PrintErrMsg(option)
  endif
      ...
      if (associated(patch%aux%MpFlow)) then
        select case(ivar)
          ...
          case(ICE_DENSITY)
            do local_id=1,grid%nlmax
              vec_ptr(local_id) = &
                patch%aux%MpFlow%auxvars(grid%nL2G(local_id))%den_kg(option%nfluid+1)
            enddo
          case default
    ...
    case(TEMPERATURE,LIQUID_PRESSURE,GAS_PRESSURE, &
      ...
      if (associated(patch%aux%MpFlow)) then
        select case(ivar)
          ...
          case(ICE_DENSITY)
            value = patch%aux%MpFlow%auxvars(ghosted_id)%den_kg(option%nfluid+1)
          case default
            call PatchUnsupportedVariable('MPFLOW',ivar,option)
        end select
...
subroutine PatchSetupUpwindDirection(patch,option)
  ...
  allocate(upwind_direction(option%nfluid,sum_connection))
  cur_connection_set => connection_set_list%first
  sum_connection = 0
  do
    if (.not.associated(cur_connection_set)) exit
  ...
  boundary_condition => patch%boundary_condition_list%first
  allocate(upwind_direction_bc(option%nfluid,sum_connection))
  sum_connection = 0
  do
    if (.not.associated(boundary_condition)) exit
    cur_connection_set => boundary_condition%connection_set
