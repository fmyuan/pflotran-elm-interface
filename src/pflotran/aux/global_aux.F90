module Global_Aux_module
  ! variables globally shared (accessible) for all
...
subroutine GlobalAuxVarInit(auxvar,option)
  ...
  nfluid = max(1,option%nfluid)  ! at least liq. fluid exists, for purpose of transport with known velocity (data-driven)
  if(nfluid>0) then
