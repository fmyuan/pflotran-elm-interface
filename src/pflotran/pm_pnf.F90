...
#include "petsc/finclude/petscsnes.h"
  ...
  contains
    ...
    procedure, public :: ReadNewtonBlock => PMPNFReadNewtonSelectCase
    procedure, public :: InitializeRun => PMPNFInitializeRun
    procedure, public :: InitializeTimestep => PMPNFInitializeTimestep
    procedure, public :: Residual => PMPNFResidual
    procedure, public :: Jacobian => PMPNFJacobian
    ...
    procedure, public :: CheckUpdatePre => PMPNFCheckUpdatePre
    procedure, public :: CheckUpdatePost => PMPNFCheckUpdatePost
    procedure, public :: CheckConvergence => PMPNFCheckConvergence
...
subroutine PMPNFReadNewtonSelectCase(this,input,keyword,found, &
                                         error_string,option)
  !
  ! Reads input file parameters associated with the PNF process model
  ...
  error_string = 'PNF Newton Solver'
  ...
  select case(trim(keyword))
    case('REL_LIQ_PRESSURE_PERTURBATION')
      call InputReadDouble(input,option,pnf_pres_rel_pert)
      call InputErrorMsg(input,option,keyword,error_string)
      ! no units conversion since it is relative
    case('MIN_LIQ_PRESSURE_PERTURBATION')
      call InputReadDouble(input,option,pnf_pres_min_pert)
      call InputErrorMsg(input,option,keyword,error_string)
      call InputReadAndConvertUnits(input,pnf_pres_min_pert, &
                                    'Pa',keyword,option)
...
end subroutine PMPNFReadNewtonSelectCase
...
subroutine PMPNFResidual(this,snes,xx,r,ierr)
  !
  ! Author: Glenn Hammond
  ! Date: 08/27/21
  !
  use PNF_module, only : PNFResidual
  use Debug_module
  use Grid_module
  ...
  if (pnf_simult_function_evals) then
    call SNESGetJacobian(snes,M,PETSC_NULL_MAT, &
                         ...
                         ierr);CHKERRQ(ierr)
    call PNFResidual(snes,xx,r,M,this%realization,ierr)
  else
    call PNFResidual(snes,xx,r,PETSC_NULL_MAT,this%realization,ierr)
  endif
...
end subroutine PMPNFResidual

! ************************************************************************** !

subroutine PMPNFJacobian(this,snes,xx,A,B,ierr)
  !
  ! Author: Glenn Hammond
  ! Date: 08/27/21
  !
  ...
  ! the Jacobian was already calculated in PMPNFResidual

  if (this%realization%debug%matview_Matrix) then
    call DebugWriteFilename(this%realization%debug,string,'PNFjacobian','', &
  ...
  pnf_ni_count = pnf_ni_count + 1

end subroutine PMPNFJacobian

! ************************************************************************** !

subroutine PMPNFCheckUpdatePre(this,snes,X,dX,changed,ierr)
  !
  ! Author: Glenn Hammond
  ! Date: 08/27/21
  !
...
end subroutine PMPNFCheckUpdatePre

! ************************************************************************** !

subroutine PMPNFCheckUpdatePost(this,snes,X0,dX,X1,dX_changed, &
                                    X1_changed,ierr)
  !
  ! Author: Glenn Hammond
  ! Date: 08/27/21
...
end subroutine PMPNFCheckUpdatePost

! ************************************************************************** !

subroutine PMPNFCheckConvergence(this,snes,it,xnorm,unorm, &
                                   fnorm,reason,ierr)
  ! Author: Glenn Hammond
  ! Date: 08/27/21
  !
...
end subroutine PMPNFCheckConvergence
