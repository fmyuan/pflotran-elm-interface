module Transport_common_module
...
subroutine TDispersion(global_auxvar_up,material_auxvar_up, &
  ...
  PetscReal :: dist(-1:3)
  PetscReal :: qdarcy(*)
  type(transport_param_type) :: tran_parameter
  PetscReal :: harmonic_tran_coefs_over_dist(tran_parameter%ncomp, &
                                             tran_parameter%nphase)
  ...
  PetscReal :: molecular_diffusion_up(tran_parameter%ncomp)
  PetscReal :: molecular_diffusion_dn(tran_parameter%ncomp)
  PetscReal :: hydrodynamic_dispersion_up(tran_parameter%ncomp)
  PetscReal :: hydrodynamic_dispersion_dn(tran_parameter%ncomp)
  PetscReal :: t_ref_inv
  PetscReal :: v_up, v_dn
  PetscReal :: vi2_over_v_up, vj2_over_v_up, vk2_over_v_up
  PetscReal :: vi2_over_v_dn, vj2_over_v_dn, vk2_over_v_dn  

  nphase = tran_parameter%nphase
  ...
  do iphase = 1, nphase
    ...
    if (sat_up < tran_min_saturation .or. sat_dn < tran_min_saturation) cycle
    if (tran_parameter%temperature_dependent_diffusion) then
      select case(iphase)
        case(LIQUID_PHASE)
          t_ref_inv = 1.d0/298.15d0 ! 1. / (25. + 273.15)
          molecular_diffusion_up(:) = &
            tran_parameter%diffusion_coefficient(:,iphase) * &
            exp(tran_parameter%diffusion_activation_energy(:,iphase) / &
                IDEAL_GAS_CONSTANT* &
    ...
    q = qdarcy(iphase)
    if (tran_parameter%calculate_transverse_dispersion) then
...
subroutine TDispersionBC(ibndtype, &
  ...
  PetscReal :: qdarcy(*)
  type(transport_param_type) :: tran_parameter
  PetscReal :: tran_coefs_over_dist(tran_parameter%ncomp, &
                                             tran_parameter%nphase)
  ...
  PetscReal :: molecular_diffusion(tran_parameter%ncomp)
  PetscReal :: hydrodynamic_dispersion(tran_parameter%ncomp)
  PetscReal :: v_dn
  PetscReal :: vi2_over_v_dn, vj2_over_v_dn, vk2_over_v_dn  
  PetscReal :: t_ref_inv
...
subroutine TFlux(tran_parameter, &
                 tran_auxvar_up,global_auxvar_up, &
                 tran_auxvar_dn,global_auxvar_dn, &
  ...
  type(transport_param_type) :: tran_parameter
  type(transport_auxvar_type) :: tran_auxvar_up, tran_auxvar_dn
  type(global_auxvar_type) :: global_auxvar_up, global_auxvar_dn 
  type(option_type) :: option
  PetscReal :: coef_up(tran_parameter%ncomp,tran_parameter%nphase)
  PetscReal :: coef_dn(tran_parameter%ncomp,tran_parameter%nphase)
  PetscReal :: Res(tran_parameter%ncomp,tran_parameter%nphase)
  ...
  ndof = tran_parameter%ncomp
  
  Res = 0.d0
  
  ! units = (L water/sec)*(mol/L) = mol/s
  ! total = mol/L water/air
  do iphase = 1, tran_parameter%nphase

    Res(1:ndof, iphase) = coef_up(1:ndof,iphase)* &
             tran_auxvar_up%aq_total(1:ndof,iphase) + &
                          coef_dn(1:ndof,iphase)* &
             tran_auxvar_dn%aq_total(1:ndof,iphase)
...
subroutine TFlux_CD(tran_parameter, &
                 tran_auxvar_up,global_auxvar_up, &
                 tran_auxvar_dn,global_auxvar_dn, &
  ...
  type(global_auxvar_type) :: global_auxvar_up, global_auxvar_dn 
  type(option_type) :: option
  PetscReal :: coef_11(tran_parameter%ncomp,tran_parameter%nphase)
  PetscReal :: coef_12(tran_parameter%ncomp,tran_parameter%nphase)
  PetscReal :: coef_21(tran_parameter%ncomp,tran_parameter%nphase)
  PetscReal :: coef_22(tran_parameter%ncomp,tran_parameter%nphase)
  PetscReal :: Res_1(tran_parameter%ncomp)
  PetscReal :: Res_2(tran_parameter%ncomp)
  ...
  ! units = (L water/sec)*(mol/L) = mol/s
  ! total(:,1) = mol/L water
  Res_1(1:ndof) = coef_11(1:ndof,iphase)*tran_auxvar_up%total(1:ndof,iphase) + &
                  coef_12(1:ndof,iphase)*tran_auxvar_dn%total(1:ndof,iphase)
  Res_2(1:ndof) = coef_21(1:ndof,iphase)*tran_auxvar_up%total(1:ndof,iphase) + &
                  coef_22(1:ndof,iphase)*tran_auxvar_dn%total(1:ndof,iphase)

  if (tran_parameter%ngas > 0) then
    iphase = 2
  ! total(:,2) = mol/L gas
    Res_1(1:ndof) = coef_11(1:ndof,iphase)*tran_auxvar_up%total(1:ndof,iphase) + &
                    coef_12(1:ndof,iphase)*tran_auxvar_dn%total(1:ndof,iphase)
...
subroutine TFluxDerivative(tran_parameter, &
                           tran_auxvar_up,global_auxvar_up, &
                           tran_auxvar_dn,global_auxvar_dn, &
  ...
  PetscReal :: coef_up(tran_parameter%ncomp,tran_parameter%nphase)
  PetscReal :: coef_dn(tran_parameter%ncomp,tran_parameter%nphase)
  PetscReal :: J_up(tran_parameter%ncomp,tran_parameter%ncomp), &
               J_dn(tran_parameter%ncomp,tran_parameter%ncomp)
  ...
  !       = kg water/sec
  istart = 1
  iendaq = tran_parameter%ncomp
  J_up = 0.d0
  J_dn = 0.d0
  do iphase = 1, nphase
    if (associated(tran_auxvar_dn%aqueous%dtotal)) then
      do irow = istart, iendaq
        J_up(irow,istart:iendaq) = &
          J_up(irow,istart:iendaq) + &
          tran_auxvar_up%aqueous%dtotal(irow,:,iphase)* &
          coef_up(irow,iphase)
        J_dn(irow,istart:iendaq) = &
          J_dn(irow,istart:iendaq) + &
          tran_auxvar_dn%aqueous%dtotal(irow,:,iphase)* &
          coef_dn(irow,iphase)
      enddo
    else  
...
subroutine TFluxDerivative_CD(tran_parameter, &
                           tran_auxvar_up,global_auxvar_up, &
                           tran_auxvar_dn,global_auxvar_dn, &
  ...
  PetscReal :: coef_21(tran_parameter%ncomp,tran_parameter%nphase)
  PetscReal :: coef_22(tran_parameter%ncomp,tran_parameter%nphase)
  PetscReal :: J_11(tran_parameter%ncomp,tran_parameter%ncomp), &
               J_12(tran_parameter%ncomp,tran_parameter%ncomp), &
               J_21(tran_parameter%ncomp,tran_parameter%ncomp), &
               J_22(tran_parameter%ncomp,tran_parameter%ncomp)
...
subroutine TFluxCoef(tran_parameter, &
  ...
  PetscReal :: T_up(tran_parameter%ncomp,tran_parameter%nphase)
  PetscReal :: T_dn(tran_parameter%ncomp,tran_parameter%nphase)

  PetscInt :: iphase, nphase
  PetscReal :: coef_up(tran_parameter%ncomp)
  PetscReal :: coef_dn(tran_parameter%ncomp)
  PetscReal :: q
  ...
  if (global_auxvar_up%sat(LIQUID_PHASE) < tran_min_saturation .or. &
      global_auxvar_dn%sat(LIQUID_PHASE) < tran_min_saturation) return
...
subroutine TFluxCoef_CD(tran_parameter, &
                        global_auxvar_up,global_auxvar_dn, &
                        option,area,velocity, &
                        tran_coefs_over_dist, &
  ...
  PetscReal :: tran_coefs_over_dist(tran_parameter%ncomp, &
                                    tran_parameter%nphase)
  PetscReal :: fraction_upwind
  PetscReal :: T_11(tran_parameter%ncomp,tran_parameter%nphase)
  PetscReal :: T_12(tran_parameter%ncomp,tran_parameter%nphase)
  PetscReal :: T_21(tran_parameter%ncomp,tran_parameter%nphase)
  PetscReal :: T_22(tran_parameter%ncomp,tran_parameter%nphase)

  PetscInt :: iphase
  PetscReal :: coef_up(tran_parameter%ncomp), coef_dn(tran_parameter%ncomp)
  PetscReal :: tempreal
  PetscReal :: advection_upwind(tran_parameter%nphase)
  PetscReal :: advection_downwind(tran_parameter%nphase)
  PetscReal :: q
...
subroutine TSrcSinkCoef(tran_parameter,global_auxvar,qsrc, &
  ...
  if (global_auxvar%sat(LIQUID_PHASE) < tran_min_saturation) return
  ...
  select case(tran_src_sink_type)
    case(EQUILIBRIUM_SS)
      ...
      T_out = -1.d0*T_in
    case(MASS_RATE_SS)
      ! in this case, tran_auxvar_bc%total actually holds the mass rate
      T_in = 0.d0
      T_out = -1.d0
    case default
...
subroutine TFluxTVD(tran_parameter,velocity,area,dist, &
                    total_up2,tran_auxvar_up, &
                    tran_auxvar_dn,total_dn2, &
                    TFluxLimitPtr, &
                    option,flux)
  ! 
  ...
  type(option_type) :: option
  PetscReal :: flux(tran_parameter%ncomp)
  ...
  do iphase = 1, tran_parameter%nphase
    ...
    if (velocity_area >= 0.d0) then
      ! mol/sec = L/sec * mol/L
      flux = velocity_area*tran_auxvar_up%total(1:tran_parameter%ncomp,iphase)
      if (associated(total_up2)) then
        do idof = 1, ndof
          dc = tran_auxvar_dn%total(idof,iphase) - &
               tran_auxvar_up%total(idof,iphase)
          if (dabs(dc) < 1.d-20) then
            theta = 1.d0
          else
            theta = (tran_auxvar_up%total(idof,iphase) - &
      ...
      flux = velocity_area*tran_auxvar_dn%total(1:tran_parameter%ncomp,iphase)
        ...
        do idof = 1, ndof
          ...
          if (dabs(dc) < 1.d-20) then
            ...
            theta = (total_dn2(idof,iphase) - &
                     tran_auxvar_dn%total(idof,iphase)) / &
                    dc
          endif
          correction = 0.5d0*velocity_area*(1.d0+nu)* &
                       TFluxLimitPtr(theta)* &
...
end module Transport_common_module
