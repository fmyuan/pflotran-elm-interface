...
#include "petsc/finclude/petscvec.h"
...
#if 0
  if (option%nfluid == 1 .and. option%transport%nphase > 1) then
  ...
  local_size = local_size/(option%nfluid*MAX_FACE_PER_CELL + 1)

  allocate(double_array(local_size*(MAX_FACE_PER_CELL+1)))
  double_array = 0.d0

  offset = option%nfluid*MAX_FACE_PER_CELL+1
...
subroutine OutputHDF5Provenance_input(option, pflotran_id)
  ...
  !
  use hdf5
  use Input_Util_module, only : input_type, InputCreate, InputDestroy, &
