...
subroutine OutputMassBalance(realization_base)
  ...
  PetscReal :: sum_local(realization_base%option_flow%nspecflow)
  PetscReal :: sum_global(realization_base%option_flow%nspecflow)
  ...
  allocate(mass_balance(option%nfluid+1, &
                        option_flow%nspecflow))

  output_option => realization_base%output_option

  if (option_flow%nspecflow<=0) return

  do ispec = 1, option_flow%nspecflow  ! not yet output those for energy (TODO)
...
110 format(100es16.8)
    ...
    if (option%nflowdof > 0) then
      if (option%myrank == option%io_rank) &
        write(fid,100,advance="no") option_flow%dt/output_option%tconv
    endif
