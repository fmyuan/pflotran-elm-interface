...
subroutine CNdegasSetup(this,reaction,option)
  ...
  word = 'CO2(aq)'
  this%ispec_co2a = GetPrimarySpeciesIDFromName(word,reaction,PETSC_FALSE,option)

  word = 'N2O(aq)'
  this%ispec_n2oa = GetPrimarySpeciesIDFromName(word,reaction,PETSC_FALSE,option)

  word = 'N2(aq)'
  this%ispec_n2a = GetPrimarySpeciesIDFromName(word,reaction,PETSC_FALSE,option)

  ! note: in general we have the following
  ! air CO2(g) <--diffusion/bulbling--> CO2(g)* in water <--degasing/dissolving--> CO2(aq) <--ionzing--> HCO3-
  word = 'CO2(g)*'
  this%ispec_co2g = GetPrimarySpeciesIDFromName(word,reaction,option)
  if(this%ispec_co2g<0) then
    ! if gas CO2 related process in water not considered, it's assumed as gas species (so that constant).
    word = 'CO2(g)'
    this%ispec_co2g = GasGetIDFromName(reaction%gas,word)
  endif
 
  word = 'N2O(g)*'
  this%ispec_n2og = GetPrimarySpeciesIDFromName(word,reaction,option)
  if(this%ispec_n2og<0) then
    word = 'N2O(g)'
    this%ispec_n2og = GasGetIDFromName(reaction%gas,word)
  endif

  word = 'N2(g)*'
  this%ispec_n2g = GetPrimarySpeciesIDFromName(word,reaction, option)
  if(this%ispec_n2g<0) then
    word = 'N2(g)'
    this%ispec_n2g = GasGetIDFromName( reaction%gas,word)
  endif

  if(this%b_fixph) then
     word = 'H+'
     this%ispec_proton = GetPrimarySpeciesIDFromName(word,reaction,option)
