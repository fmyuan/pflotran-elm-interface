module Reaction_Sandbox_CNdegas_class
...
#include "petsc/finclude/petscsys.h"
  ...
  type, public, &
    extends(reaction_sandbox_base_type) :: reaction_sandbox_CNdegas_type
    ...
    PetscReal :: k_kinetic_n2o, k_kinetic_n2
    PetscReal :: k_kinetic_h
    PetscBool :: b_fixph
    PetscReal :: fixph

  contains
    procedure, public :: ReadInput => CNdegasRead
    procedure, public :: Setup => CNdegasSetup
    procedure, public :: Evaluate => CNdegasReact
    procedure, public :: Destroy => CNdegasDestroy
  end type reaction_sandbox_CNdegas_type

  public :: CNdegasCreate

contains

! ************************************************************************** !
!
! CNdegasCreate: Allocates CO2/NOx/N2 degas reaction object.
!
! ************************************************************************** !
function CNdegasCreate()

  implicit none
  
  class(reaction_sandbox_CNdegas_type), pointer :: CNdegasCreate

! 4. Add code to allocate object and initialized all variables to zero and
!    nullify all pointers. E.g.
  allocate(CNdegasCreate)
  CNdegasCreate%ispec_co2a = 0
  CNdegasCreate%ispec_co2g = 0
  CNdegasCreate%ispec_n2oa = 0
  CNdegasCreate%ispec_n2og = 0
  CNdegasCreate%ispec_n2a = 0
  CNdegasCreate%ispec_n2g = 0
  CNdegasCreate%ispec_himm = 0
  CNdegasCreate%ispec_proton = 0
  CNdegasCreate%k_kinetic_co2 = 1.d-5
  CNdegasCreate%k_kinetic_n2o = 1.d-5
  CNdegasCreate%k_kinetic_n2 = 1.d-5
  CNdegasCreate%k_kinetic_h = 1.d-5
  CNdegasCreate%fixph = 6.5d0
  CNdegasCreate%b_fixph = PETSC_FALSE
  nullify(CNdegasCreate%next)
      
end function CNdegasCreate

! ************************************************************************** !
!
! CNdegasRead: Reads input deck for CNdegas reaction parameters (if any)
!
! ************************************************************************** !
subroutine CNdegasRead(this,input,option)
  ...
  class(reaction_sandbox_CNdegas_type) :: this
  ...
  do 
    ...
    call InputErrorMsg(input,option,'keyword', &
                       'CHEMISTRY,REACTION_SANDBOX,CNdegas')
    ...
    select case(trim(word))
      case('KINETIC_CONSTANT_CO2')
         ...
         call InputErrorMsg(input,option,'CO2 degas kinetic rate constant', &
                     'CHEMISTRY,REACTION_SANDBOX,CNdegas,REACTION')
      ...
      case default
          option%io_buffer = 'CHEMISTRY,REACTION_SANDBOX,CNdegas,' // &
            'REACTION keyword: ' // trim(word) // ' not recognized.'
          call printErrMsg(option)
    end select
  enddo
  
end subroutine CNdegasRead

! ************************************************************************** !
!
! CNdegasSetup: Sets up the CNdegas reaction either with parameters either
!                read from the input deck or hardwired.
! author: Guoping Tang
! date: 04/03/2014
!
! ************************************************************************** !
subroutine CNdegasSetup(this,reaction,option)
  ...
  word = 'CO2(aq)'
  this%ispec_co2a = ReactionAuxGetPriSpecIDFromName(word,reaction,PETSC_FALSE,option)

  word = 'N2O(aq)'
  this%ispec_n2oa = ReactionAuxGetPriSpecIDFromName(word,reaction,PETSC_FALSE,option)

  word = 'N2(aq)'
  this%ispec_n2a = ReactionAuxGetPriSpecIDFromName(word,reaction,PETSC_FALSE,option)

  ! note: in general we have the following
  ! air CO2(g) <--diffusion/bulbling--> CO2(g)* in water <--degasing/dissolving--> CO2(aq) <--ionzing--> HCO3-
  word = 'CO2(g)*'
  this%ispec_co2g = ReactionAuxGetPriSpecIDFromName(word,reaction,option)
  if(this%ispec_co2g<0) then
    ! if gas CO2 related process in water not considered, it's assumed as gas species (so that constant).
    word = 'CO2(g)'
    this%ispec_co2g = ReactionGasGetGasIDFromName(reaction%gas,word)
  endif
 
  word = 'N2O(g)*'
  this%ispec_n2og = ReactionAuxGetPriSpecIDFromName(word,reaction,option)
  if(this%ispec_n2og<0) then
    word = 'N2O(g)'
    this%ispec_n2og = ReactionGasGetGasIDFromName(reaction%gas,word)
  endif

  word = 'N2(g)*'
  this%ispec_n2g = ReactionAuxGetPriSpecIDFromName(word,reaction, option)
  if(this%ispec_n2g<0) then
    word = 'N2(g)'
    this%ispec_n2g = ReactionGasGetGasIDFromName( reaction%gas,word)
  endif

  if(this%b_fixph) then
     word = 'H+'
     this%ispec_proton = ReactionAuxGetPriSpecIDFromName(word,reaction,option)

     word = 'Himm'
     this%ispec_himm = ReactionImGetSpeciesIDFromName( &
...
end subroutine CNdegasSetup

! ************************************************************************** !
!
! CNdegasReact: Evaluates reaction storing residual and/or Jacobian
!
! ************************************************************************** !
subroutine CNdegasReact(this,Residual,Jacobian,compute_derivative, &
                         rt_auxvar,global_auxvar,material_auxvar,reaction, &
                         option)

  use Option_module
  use Reaction_Aux_module
  !use TH_Aux_module   !(TODO) to obtain ice saturation state
  ...
  !type(TH_auxvar_type) :: th_auxvar  !(TODO) to obtain ice saturation state
...
!-------------------------------------------------------------------------------------
  ...
  elseif (option%ntrandof.gt.0 ) then
      ...
      option%io_buffer='reaction_sandbox_CNdegas ' // &
...
!
  ...
  do ires=1, reaction%ncomp
    ...
    if (abs(temp_real) > huge(temp_real)) then
      ...
      write(option%fid_out, *) 'Reaction Sandbox: CNdegas'

      option%io_buffer = 'checking infinity of Residuals matrix @ CNdegasReact '
      call printErrMsg(option)
    endif
    ...
    if (temp_real /= temp_real) then
      ...
      write(option%fid_out, *) 'Reaction Sandbox: CNdegas'
      option%io_buffer = ' checking NaN of Residuals matrix  @ CNdegasReact '
      call printErrMsg(option)
    endif
...
end subroutine CNdegasReact

! ************************************************************************** !
!
! CNdegasDestroy: Destroys allocatable or pointer objects created in this
!                  module
!
! ************************************************************************** !
subroutine CNdegasDestroy(this)

  implicit none
  
  class(reaction_sandbox_CNdegas_type) :: this

end subroutine CNdegasDestroy
...
end module Reaction_Sandbox_CNdegas_class
