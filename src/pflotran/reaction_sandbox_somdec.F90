...
! -----------------------------------------------------------------------------
  ...
  type, public, &
    ...
    character(len=MAXWORDLENGTH) :: NxOy_species  ! NOx species name in PFLOTRAN's reaction network (gas, gaseous in solution, aqueous, OR ionized aqueous)
  ...
  type, public :: somdec_reaction_type
    ...
    type(inhibition2_type), pointer :: inhibition
...
function InhibitionCreate()
  ...
  type(inhibition2_type), pointer :: InhibitionCreate
              ...
              if (InputError(input)) then
                ...
                rate_constant = rate_constant * &
                  UnitsConvertToInternal(word,internal_units, &
                                         'REACTION_SANDBOX,SOMDEC,RATE CONSTANT UNITS', &
                                         option)
              endif
            case('TURNOVER_TIME')
              internal_units = 'sec'
              ...
              if (InputError(input)) then
                ...
                turnover_time = turnover_time * &
                  UnitsConvertToInternal(word,internal_units,&
                                         'REACTION_SANDBOX,SOMDEC,TURNOVER TIME UNITS', &
                                         option)
              endif
            case('RATE_DECOMPOSITION')
              ...
              if (InputError(input)) then
                ...
                rate_decomposition = rate_decomposition * &
                  UnitsConvertToInternal(word,internal_units,&
                                         'REACTION_SANDBOX,SOMDEC,RATE DECOMPOSITION UNITS', &
                                         option)
              endif
            case('RATE_AD_FACTOR')
...
subroutine SomDecSetup(this,reaction,option)
  ...
  type(inhibition2_type), pointer :: cur_inhibition
  PetscInt :: ires
