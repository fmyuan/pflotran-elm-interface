...
! -----------------------------------------------------------------------------
  ...
  type, public, &
    ...
    character(len=MAXWORDLENGTH) :: NOx_species   ! NOx species name in PFLOTRAN's reaction network (gas, gaseous in solution, aqueous, OR ionized aqueous) for soil N emission reaction
  ...
  type, public :: monod2_type                   ! note this is different from what in reaction_microbial_aux.F90
  ...
  type, public :: inhibition2_type
      PetscInt :: specid
      PetscInt :: specitype
      type(inhibition_type), pointer :: inhibition

      type(inhibition2_type), pointer :: next
  end type inhibition2_type
  ...
  type, public :: somdec_reaction_type
    ...
    type(monod2_type), pointer :: rxn_monod
    type(inhibition2_type), pointer :: rxn_inhibition
...
function RxnMonodCreate()
  ...
  type(monod2_type), pointer :: RxnMonodCreate

  type(monod2_type), pointer :: RxnMonod

  allocate(RxnMonod)
  RxnMonod%species_name = ''
  RxnMonod%specid = UNINITIALIZED_INTEGER
...
end function RxnMonodCreate

! ************************************************************************** !

function RxnInhibitionCreate()
  !
  ! Allocate and initialize an extended inhibition object within SomDec reaction
  ! By: Fengming Yuan @2019-06-20

  implicit none

  type(inhibition2_type), pointer :: RxnInhibitionCreate

  type(inhibition2_type), pointer :: RxnInhibition

  allocate(RxnInhibition)

  RxnInhibition%inhibition => ReactionInhibitionCreateAux()

  ! extended variables
  RxnInhibition%specid = UNINITIALIZED_INTEGER
  RxnInhibition%specitype = UNINITIALIZED_INTEGER
  nullify(RxnInhibition%next)

  RxnInhibitionCreate => Rxninhibition
end function RxnInhibitionCreate
...
subroutine SomDecRead(this,input,option)
  ...
  type(monod2_type), pointer :: new_monod, prev_monod
  type(inhibition2_type), pointer :: new_inhibition, prev_inhibition
    ...
    select case(trim(word))
     ...
     case('POOLS')
      ...
      case('REACTION')
        ...
        nullify(new_rxn%rxn_monod)
        nullify(new_rxn%rxn_inhibition)
        ...
        do 
          ...
          select case(trim(word))
            ...
            case('RATE_CONSTANT')
              ...
              if (InputError(input)) then
                ...
                rate_constant = rate_constant * &
                  UnitsConvertToInternal(word,internal_units,'SomDec,REACTION,RATE_CONSTANT', &
                  option)
              endif
            case('TURNOVER_TIME')
              internal_units = 'sec'
              ...
              if (InputError(input)) then
                ...
                turnover_time = turnover_time * &
                  UnitsConvertToInternal(word,internal_units,'SomDec,REACTION,TURNOVER_TIME', &
                  option)
              endif
            case('RATE_DECOMPOSITION')
              ...
              if (InputError(input)) then
                ...
                rate_decomposition = rate_decomposition * &
                  UnitsConvertToInternal(word,internal_units,'SomDec,REACTION,RATE_DECOMPOSITION', &
                  option)
              endif
            case('RATE_AD_FACTOR')
            ...
            case('MONOD')
              ...
              if (.not.associated(new_rxn%rxn_monod)) then
                new_rxn%rxn_monod => new_monod
              else
            ...
            case('INHIBITION')
              new_inhibition => RxnInhibitionCreate()
              call ReactionInhibitionRead(new_inhibition%inhibition,input,option, &
                     trim(new_rxn%upstream_pool_name), &
                     'CHEMISTRY,REACTION_SANDBOX,SomDec,REACTION,INHIBITION')

              ! create or append to list
              if (.not.associated(new_rxn%rxn_inhibition)) then
                new_rxn%rxn_inhibition => new_inhibition
              else
...
subroutine SomDecSetup(this,reaction,option)
  ...
  type(monod2_type), pointer :: cur_monod
  type(inhibition2_type), pointer :: cur_rxninhibit
  PetscInt :: ires
...
! pools
  ...
  do
    ...
    ! only one species name needed for pools with fixed N/C ratio (e.g. SOMX)
      ...
      word=trim(cur_pool%name)
      species_id_pool_c(icount) = &
        ReactionImGetSpeciesIDFromName(word,reaction%immobile,PETSC_FALSE,option)
      if(species_id_pool_c(icount) <= 0) then
          species_id_pool_c(icount) = &
            ReactionAuxGetPriSpecIDFromName(cur_pool%name, reaction, option)
          if(species_id_pool_c(icount) <= 0) then
...
! reactions
  ...
  do
    ...
    cur_rxninhibit => cur_rxn%rxn_inhibition
    do
      if (.not. associated(cur_rxninhibit)) exit
      call SomDec_SpeciesID(this, reaction, cur_rxninhibit%inhibition%species_name, &
        cur_rxninhibit%specid, cur_rxninhibit%specitype, option)

      cur_rxninhibit => cur_rxninhibit%next
    enddo
  ...
  ! not specified (then have to test possible species)
    ...
    this%species_id_co2 = ReactionAuxGetPriSpecIDFromName(word,reaction, PETSC_FALSE, option)
    ...
    if (this%species_id_co2 <= 0) then
      ...
      if (this%species_id_co2 <= 0) then
        ...
        if(this%species_id_co2 <= 0) then
          ...
          this%species_id_co2 = ReactionGasGetGasIDFromName(reaction%gas, word)
  ...
  !
  word = 'NH4+'
  this%species_id_nh4 = ReactionAuxGetPriSpecIDFromName(word, reaction, PETSC_FALSE, option)
#ifdef ELM_PFLOTRAN
  ...
  word = 'NO3-'
  this%species_id_no3 = ReactionAuxGetPriSpecIDFromName(word, reaction, PETSC_FALSE, option)
  
  ! for soil N emission reactions
  word = trim(this%NOx_species)
  if (len_trim(word) > 0) then
    call SomDec_SpeciesID(this, reaction, word, &
      this%species_id_co2, this%species_itype_co2, option)
  ...
  ! not specified (then have to test possible species)
    this%species_itype_n2o = ITYPE_AQUEOUS !by default
    word = 'N2O(g)*'     ! N2O gas in water, ie CO2(g)* <-> CO2(aq) by dissolving/degasing
    this%species_id_n2o = ReactionAuxGetPriSpecIDFromName(word,reaction, PETSC_FALSE, option)

    if (this%species_id_n2o <= 0) then
      word = 'N2O(aq)' ! N2O in aquious phase
      this%species_id_n2o = ReactionAuxGetPriSpecIDFromName(word,reaction, PETSC_FALSE, option)

      if(this%species_id_n2o <= 0) then
          word = 'N2O(g)'    ! N2O gas in air, ie N2O(g) <-> N2O(g)*, by diffusion/bulbling, <-> N2O(aq) by dissolving/degasing
          this%species_id_n2o = ReactionGasGetGasIDFromName(reaction%gas, word)
          this%species_itype_n2o = ITYPE_GAS

          if (this%species_id_n2o <= 0) then
            word = ''
            this%species_itype_n2o = -1
            option%io_buffer = 'N2O(g)* or N2O(aq) or N2O(g) is not specified in the input file for N emission reaction!'
            call printMsg(option)
          endif
    ...
    this%NOx_species = trim(word)
  endif

  word = 'H+'
  this%species_id_proton = ReactionAuxGetPriSpecIDFromName( &
            word,reaction,PETSC_FALSE,option)

  word = 'HRimm'
  this%species_id_hr = ReactionImGetSpeciesIDFromName( &
            word,reaction%immobile,PETSC_FALSE,option)

  word = 'Nmin'
  this%species_id_nmin = ReactionImGetSpeciesIDFromName( &
            word,reaction%immobile,PETSC_FALSE,option)
 
  word = 'Nimp'
  this%species_id_nimp = ReactionImGetSpeciesIDFromName( &
            word,reaction%immobile,PETSC_FALSE,option)

  word = 'Nimm'
  this%species_id_nimm = ReactionImGetSpeciesIDFromName( &
            word,reaction%immobile,PETSC_FALSE,option)
 
  word = 'NGASmin'
  this%species_id_ngasmin = ReactionImGetSpeciesIDFromName( &
            word,reaction%immobile,PETSC_FALSE,option)
...
! misc. local variables
  ...
  rxninhibit => rxn%rxn_inhibition
  do
    if(.not.associated(rxninhibit)) exit

    call SomDec_SpeciesConc(this, reaction, rt_auxvar,  &
               rxninhibit%specid, rxninhibit%specitype, &
               tempreal, option)
    inhibition_k = rxninhibit%inhibition%inhibition_constant
    inhibition_k2 = rxninhibit%inhibition%inhibition_constant2

    select case(rxninhibit%inhibition%itype)
      case(INHIBITION_MONOD)
        call ReactionInhibitionMonod(tempreal,inhibition_k, &
                                     fx,dfx)
      case(INHIBITION_THRESHOLD)
        call ReactionInhibitionThreshold(tempreal,inhibition_k, &
                                        inhibition_k2, &
                                        fx,dfx)
      case(INHIBITION_SMOOTHSTEP)
        call ReactionInhibitionSmoothstep(tempreal,inhibition_k, &
                                        inhibition_k2, &
                                        fx,dfx)
    end select

    if(ispec_uc /= rxninhibit%specid) dfx = 0.d0            ! NOT sure this is needed

    dfmb = dfmb*fx + fmb*dfx
    fmb = fmb*fx                ! do this after 'df'

    rxninhibit => rxninhibit%next
  end do
  !
...
! ----------------------------------------------------------------------------!
  ...
  if(this%species_itype_n2o == ITYPE_AQUEOUS) then
    ires_n2o = this%species_id_n2o + reaction%offset_aqueous       ! as aq. species
  elseif(this%species_itype_n2o == ITYPE_GAS) then
    ires_n2o = this%species_id_n2o   ! gas species NOT in all vectors (? TODO checking)
  endif
...
subroutine SomDecDestroy(this)
  ...
  type(monod2_type), pointer :: cur_monod, prev_monod
  type(inhibition2_type), pointer :: cur_inhibition, prev_inhibition
  ...
  do
    ...
    cur_inhibition => cur_rxn%rxn_inhibition
    do
...
subroutine SomDec_SpeciesID(this, reaction, species_name, &
  ...
  ! Primary species
  species_id = ReactionAuxGetPriSpecIDFromName(word,reaction,PETSC_FALSE,option)
  ...
  if(species_id > 0) then
    ...
    ! Gas species
    species_id = ReactionGasGetGasIDFromName(reaction%gas,word)
    if (species_id > 0) then
      ...
      ! Immobile species
      species_id = ReactionImGetSpeciesIDFromName(word, &
