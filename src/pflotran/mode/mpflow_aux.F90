...
function MpFlowAuxCreate(option_flow)
  ! 
...
subroutine MpFlowAuxVarInit(auxvar,option_flow)
  ! 
  ...
  !
  call AuxVarFlowEnergyInit(auxvar,option_flow)

  !
  auxvar%air_pressure  = UNINITIALIZED_DOUBLE
  auxvar%pres_fh2o     = UNINITIALIZED_DOUBLE
  nullify(auxvar%kvr)
  allocate(auxvar%kvr(option_flow%nfluid))
  ...
  if (.not.option_flow%numerical_derivatives) then
    auxvar%has_derivs = PETSC_TRUE    ! not needed, but just in case

    allocate(auxvar%dpres_fh2o(option_flow%nflowdof))
    auxvar%dpres_fh2o    = UNINITIALIZED_DOUBLE

    allocate(auxvar%dkvr(option_flow%nfluid, option_flow%nflowdof))
    auxvar%dkvr          = UNINITIALIZED_DOUBLE
    allocate(auxvar%dDk_eff(option_flow%nflowdof))
...
#include "petsc/finclude/petscsys.h"
  ...
  type(option_flow_type) :: option_flow
  PetscReal, intent(in) :: x(option_flow%nflowdof)
  ...
  PetscBool :: DTRUNC_FLAG = PETSC_TRUE  ! option_flow for truncating deriatives to zero at bounds (default: TRUE)
  ...
  presg  = option_flow%reference_pressure   ! presg: air pressure for air_fluid movement (TODO: the diff is PC)
  Tk     = x(IFDOF2)+TC2TK

  ! isothermal, i.e. hydrology only
  if(option_flow%isothermal) TK = option_flow%reference_temperature+TC2TK
  ! option_flow%isobaric, i.e. thermal process only
  if(option_flow%isobaric) then
    presl = option_flow%reference_pressure
    presg = option_flow%reference_pressure
  endif
...
!***************  3-phase water properties **********************************************************
  ...
  auxvar%den_kg(option_flow%nfluid+1)          = den_ice*FMWH2O  ! '%den' in kmol/m3 ONLY defined for fluids
  if (auxvar%has_derivs) then
    auxvar%D_den_kg(option_flow%nfluid+1,IFDOF1) = dden_ice_dp*FMWH2O
    auxvar%D_den_kg(option_flow%nfluid+1,IFDOF2) = dden_ice_dt*FMWH2O
  endif
  ...
  auxvar%U(option_flow%nfluid+1)           = u * 1.d-6         ! J/kmol -> MJ/kmol
  if (auxvar%has_derivs) then
    auxvar%D_U(option_flow%nfluid+1,IFDOF1) = u_dp * 1.d-6
    auxvar%D_U(option_flow%nfluid+1,IFDOF2) = u_dt * 1.d-6
  endif
  ...
  auxvar%H(option_flow%nfluid+1)           = h * 1.d-6         ! J/kmol -> MJ/kmol
  if (auxvar%has_derivs) then
    auxvar%D_H(option_flow%nfluid+1, IFDOF1) = h_dp * 1.d-6
    auxvar%D_H(option_flow%nfluid+1,IFDOF2)  = h_dt * 1.d-6
  endif
...
subroutine MpFlowAuxVarComputeCharacteristicCurves( presl, presg, tc,       &
  ...
  if (option_flow%ice_model /= UNINITIALIZED_INTEGER) then
    ...
    select case (option_flow%ice_model)
  ...
  if ((sl-1.d0)>1.d-15 .or. sl<-1.d-15) then
    ...
    option_flow%io_buffer = 'MpFlow with ice mode: LIQUID Saturation error: >1 or <0'
    call printErrMsg(option)
  endif
  ...
  call characteristic_curves%liq_rel_perm_function%RelativePermeability(sl, kr, dkr_dsl, option_flow)
  dkr_dpl= dkr_dsl*dsl_dpl
  dkr_dt = dkr_dsl*dsl_dt

#else
  call characteristic_curves%liq_rel_perm_function &
                            %RelativePermeability(icedpore_sl, kr, dkr_dsl, option)
...
subroutine DarcyFlowDerivative(mpflow_auxvar_up,    mpflow_auxvar_dn,     &
  ...
  PetscReal, intent(out) :: dq_up(option_flow%nflowdof)
  PetscReal, intent(out) :: dq_dn(option_flow%nflowdof)
  ...
  PetscInt :: idof
  PetscReal :: gravity, dgravity_up(option_flow%nflowdof), dgravity_dn(option_flow%nflowdof)
  PetscReal :: dphi, ddphi_up(option_flow%nflowdof), ddphi_dn(option_flow%nflowdof)
  PetscReal :: ukvr, dukvr_up(option_flow%nflowdof), dukvr_dn(option_flow%nflowdof)
  PetscReal :: den, dden_up(option_flow%nflowdof), dden_dn(option_flow%nflowdof)  ! density in kmol/m3
  ...
  !-------------------------------------------------------------------------
  call ConnectionCalculateDistances(dist,option_flow%gravity,dd_up,dd_dn, &
  ...
  if (mpflow_auxvar_up%sat(LIQ_FLUID) > mpflow_parameter_up%sr(LIQ_FLUID) .or.  &
    ...
    if(ifderivative) then
      do idof = 1, option_flow%nflowdof
...
subroutine AdvectionDerivative(mpflow_auxvar_up,   mpflow_auxvar_dn,   &
  ...
  PetscReal, intent(in) :: dq_up(option_flow%nflowdof), dq_dn(option_flow%nflowdof)
  PetscBool, intent(in) :: ifderivative
  PetscReal, intent(out) :: qe  ! unit: MJ/sec, + from up -> dn; - from dn -> up
  PetscReal, intent(out) :: dqe_up(option_flow%nflowdof), dqe_dn(option_flow%nflowdof)

  ! local variables

  PetscInt :: idof
  PetscReal :: uh,  duh_up(option_flow%nflowdof), duh_dn(option_flow%nflowdof)
...
subroutine ConductionDerivative(mpflow_auxvar_up,   mpflow_auxvar_dn,  &
  ...
  PetscReal :: dDk_up(option_flow%nflowdof), dDk_dn(option_flow%nflowdof)
...
subroutine VaporDiffusionDerivative(auxvar_up,global_auxvar_up, &
  ...
  type(option_flow_type) :: option_flow
  PetscReal :: Jup(option_flow%nflowdof,option_flow%nflowdof), Jdn(option_flow%nflowdof,option_flow%nflowdof)
  ...
  dmolg_dt_dn = 0.d0
    ...
    if ((satg_up > eps) .and. (satg_dn > eps)) then
      ...
      p_ref = option_flow%reference_pressure   ! in Pa
