...
#include "petsc/finclude/petscsys.h"
  ...
  PetscBool, public :: sco2_spycher_simple = PETSC_TRUE
  ...
  PetscInt, public :: permeability_reduction_model = ONE_INTEGER
...
subroutine SCO2AuxVarPerturb(sco2_auxvar, global_auxvar, material_auxvar, &
  ...
  PetscReal :: cell_pressure, cap_entry_pressure, sgt_max
  PetscInt :: idof

  ! Phase ID's
  PetscInt :: lid, gid, pid, pwid, stid, spid, tgid
  ! Component ID's
  ...
  dxs = 1.0d-5 * xsl !+ min_perturbation
  ...
  select case(global_auxvar%istate)
    case(SCO2_LIQUID_STATE)
      ...
      call SCO2BrineSaturationPressure(sco2_auxvar(ZERO_INTEGER)%temp, xsl, &
                                       Psat)
      Prvap = Psat
      Pv = sco2_auxvar(ZERO_INTEGER)%pres(vpid)
      call SCO2Equilibrate(sco2_auxvar(ZERO_INTEGER)%temp,cell_pressure, &
                           sco2_auxvar(ZERO_INTEGER)%pres(co2_pressure_id), &
                           Pv, &
                           Psat, &
                           sco2_auxvar(ZERO_INTEGER)%pres(rvpid), &
...
subroutine SCO2AuxVarUpdateState(x, sco2_auxvar, global_auxvar, &
  ...
  PetscReal, parameter :: epsilon_x = 1.d-4
  PetscReal, parameter :: peta = 1.d-1

  PetscReal :: cap_entry_pressure, cell_pressure, sg_min
  ...
  PetscReal :: den_brine_kg, den_mol
  ...
  !MAN: a lot of these calculations might be redundant
  !     if they're wrapped up in AuxVarCompute
  !MAN: Don't think we should be modifying the auxvars here
  !Compute salt solubility and surface tension
  ...
  select type(sf => characteristic_curves%saturation_function)
    class is (sat_func_vg_type)
      ...
      call characteristic_curves%saturation_function% &
           Saturation(cap_entry_pressure, sl_temp, dsat_dpres, option)
      ! Saturation actually outputs liquid saturation
      sg_min = 1.d0 - sl_temp
    class default
      cap_entry_pressure = 0.d0
      sg_min = 0.d0
  ...
  select case(global_auxvar%istate)
      ...
      if (sco2_auxvar%xmass(co2_id,lid) > xco2l) then 
        ...
        if (sg_est < sg_min) then
          ...
          Pg = sco2_auxvar%pres(lid) + &
                cap_entry_pressure / beta_gl - epsilon_x
        else
          sl_temp = 1.d0 - min(sg_est, 0.d0)
          sgt_temp = 0.d0
          ! MAN: I think this is redundant
          call SCO2ComputeSurfaceTension(sco2_auxvar%temp, &
                                         xsl, sigma)
          beta_gl = CO2_REFERENCE_SURFACE_TENSION / sigma
          ...
          call SCO2ComputePcHysteresis(characteristic_curves, &
                                       ...
                                       Pc, option)
          Pc = Pc / beta_gl                             
          Pc = min(Pc,cap_entry_pressure / beta_gl + 1.d5)
    ...
    case(SCO2_GAS_STATE)
      call SCO2ComputeSaltSolubility(sco2_auxvar%temp, salt_solubility)
      !MAN: need to make sure m_salt(2) gets updated in AuxVarCompute
      !Check if there's any salt
      if (sco2_auxvar%m_salt(2) > epsilon) then
        xsl = salt_solubility
      else
        xsl = 0.d0
    ...
    case(SCO2_LIQUID_GAS_STATE)
      ...
      call SCO2BrineDensity(sco2_auxvar%temp, cell_pressure, xsl, &
                            den_brine_kg,option)
      call SCO2VaporPressureBrine(sco2_auxvar%temp, sco2_auxvar%pres(spid), &
                                  Pc,den_brine_kg, xsl,Prvap)
      Pv = Prvap
      Pco2 = max(cell_pressure - Prvap,0.d0)
      call SCO2Equilibrate(sco2_auxvar%temp,cell_pressure, &
                           sco2_auxvar%pres(co2_pressure_id), &
                           Pv, &
                           Psat, &
      ...
      elseif (sl_temp < epsilon .and. &
        ...
        call SCO2ComputeSurfaceTension(sco2_auxvar%temp, xsl, sigma)
        beta_gl = CO2_REFERENCE_SURFACE_TENSION / sigma
        sl_temp = 0.d0
        sgt_temp = 0.d0
        call SCO2ComputePcHysteresis(characteristic_curves, &
        ...
        call SCO2BrineDensity(sco2_auxvar%temp, cell_pressure, xsl, &
                              den_brine_kg,option)
        call SCO2VaporPressureBrine(sco2_auxvar%temp, sco2_auxvar%pres(spid), &
                                   sco2_auxvar%pres(cpid), den_brine_kg, xsl, &
                                   Prvap)
        sco2_auxvar%pres(co2_pressure_id) = sco2_auxvar%pres(gid) - &
                                            Prvap
      else
        ! No state transition
        istatechng = PETSC_FALSE
...
subroutine SCO2AuxVarCompute(x,sco2_auxvar,global_auxvar,material_auxvar, &
  ...
  ! Phase ID's
  PetscInt :: lid, gid, pid, pwid, pgid, stid
  ! Component ID's
  PetscInt :: wid, co2_id, co2_pressure_id, sid
  ! Other ID's
  PetscInt :: cpid, vpid, rvpid, spid, tgid

  PetscReal :: cell_pressure, Pc_scaled
  PetscReal :: xco2g, xwg, xco2l, xsl, xwl, xmolco2g, xmolwg, xmolco2l, &
               xmolsl, xmolwl
  PetscReal :: mw_mix
  ...
  select case(global_auxvar%istate)
    case(SCO2_LIQUID_STATE)
      ...
      ! Brine density
      call SCO2BrineDensity(sco2_auxvar%temp, sco2_auxvar%pres(lid), &
                            x_salt_dissolved, den_brine_kg, option)
      ...
      call SCO2WaterDensity(sco2_auxvar%temp,sco2_auxvar%pres(rvpid), &
                            ...
                            sco2_auxvar%den_kg(stid),option)
      sco2_auxvar%pres(co2_pressure_id) = max(sco2_auxvar%pres(lid) - &
                                              sco2_auxvar%pres(rvpid), 0.d0)
    ...
    case (SCO2_GAS_STATE)
      ...
      sco2_auxvar%sat(lid) = epsilon
      sco2_auxvar%sat(gid) = 1.d0 - sco2_auxvar%sat(pid) - sco2_auxvar%sat(lid)
      ...
      sco2_auxvar%xmass(gid,lid) = xco2l
      sco2_auxvar%xmass(wid,lid) = 1.d0 - sco2_auxvar%xmass(gid,lid) - &
                                   sco2_auxvar%xmass(sid,lid)
      sco2_auxvar%xmol(gid,lid) = xmolco2l
      sco2_auxvar%xmol(sid,lid) = xmolsl
      sco2_auxvar%xmol(wid,lid) = 1.d0 - sco2_auxvar%xmol(gid,lid) - &
                                  sco2_auxvar%xmol(sid,lid)
    ...
    case (SCO2_TRAPPED_GAS_STATE)
      ...
      call SCO2WaterDensity(sco2_auxvar%temp,sco2_auxvar%pres(rvpid), &
                            TWO_INTEGER,sco2_auxvar%den_kg(pwid), &
                            sco2_auxvar%den_kg(stid),option)
      sco2_auxvar%pres(co2_pressure_id) = max(sco2_auxvar%pres(gid) - &
                                              sco2_auxvar%pres(rvpid), 0.d0)
      sco2_auxvar%pres(vpid) = sco2_auxvar%pres(rvpid)
      ! Compute equilibrium mass and mole fractions for all components
      xsl = x_salt_dissolved
      ...
      sco2_auxvar%sat(lid) = 1.d0 - sco2_auxvar%sat(gid)
      ! MAN: Could this lead to major issues as liquid saturation --> 0
      !      with salt precipitate
      sco2_auxvar%sat(pid) = max((sco2_auxvar%m_salt(1) - xsl),0.d0)* &
                             sco2_auxvar%sat(lid) * den_brine_kg / &
                             sco2_auxvar%den_kg(pid)
    ...
    case (SCO2_LIQUID_GAS_STATE)
      ...
      sco2_auxvar%pres(cpid) = sco2_auxvar%pres(gid) - sco2_auxvar%pres(lid)
      ...
      call SCO2ComputeSurfaceTension(sco2_auxvar%temp, &
                                     x_salt_dissolved, sigma)
      beta_gl = CO2_REFERENCE_SURFACE_TENSION / sigma
      ! Divide out the surface tension adjustment
      Pc_scaled = sco2_auxvar%pres(cpid) / beta_gl
      ...
      sco2_auxvar%sat(pid) = max((sco2_auxvar%m_salt(1) - xsl),0.d0)* &
                             sl_temp * den_brine_kg / &
                             sco2_auxvar%den_kg(pid)
      ! Rescale to include salt
      sco2_auxvar%sat(lid) = sl_temp * (1.d0 - sco2_auxvar%sat(pid))
      sco2_auxvar%sat(gid) = 1.d0 - sco2_auxvar%sat(lid) - sco2_auxvar%sat(pid)
  ...
  sco2_auxvar%sat(gid) = (1.d0 - sco2_auxvar%sat(lid)) + sco2_auxvar%sat(tgid)
  ...
  ! Gas phase density
  call EOSGasDensity(sco2_auxvar%temp,sco2_auxvar%pres(co2_pressure_id), &
                     den_mol,drho_dT,drho_dP,ierr)
  ...
  sco2_auxvar%den_kg(gid) = sco2_auxvar%xmass(co2_id,gid) * &
                            ...
                            sco2_auxvar%den_kg(stid)
  ! Gas phase viscosity
  call SCO2ViscosityWater(sco2_auxvar%temp,sco2_auxvar%pres(vpid), &
                          sco2_auxvar%den_kg(stid),visc_water,option)
  ...
  ! Correct the liquid density to account for CO2
  call SCO2DensityCompositeLiquid(sco2_auxvar%temp,den_brine_kg, &
                                  sco2_auxvar%xmass(co2_id,lid), &
                                  sco2_auxvar%den_kg(lid))
  ...
  ! Precipitate phase is pure salt
  sco2_auxvar%xmass(sid,sid) = 1.d0
  sco2_auxvar%xmol(sid,sid) = 1.d0
  ...
  ! Salt precipitate and mineral precipitation effects
  ...
  ! Diffusivities
  call SCO2VaporDiffusionCoeff(sco2_auxvar%temp,cell_pressure, &
  ...
  sco2_auxvar%U(stid) = sco2_auxvar%H(stid) - &
                        sco2_auxvar%pres(vpid) / sco2_auxvar%den_kg(stid)
  ...
  if (sco2_auxvar%m_salt(2) == 0.d0) then
...
subroutine SCO2Equilibrate(T,P,p_co2,p_vap,p_sat,p_vap_brine, &
  ...
  if (sco2_spycher_simple) then
    ...
    xmolwg = max(min(xwg,1.d0),0.d0)

    Hc = 6.305d-4 * exp(2.4d3*((1.d0/T_k)-(1.d0/298.15d0)))
    p_co2 = max(P-p_vap,0.d0)/1.d5
  ...
  ! MAN: gets a little circular, since p_co2 is initially computed with 
  !      the reduced vapor pressure and is used in intermediate calcs.
  p_co2 = xmolco2g * P
  p_water = xmolwg * P
...
subroutine SCO2WaterDensity(T,P,isubr,rho_l,rho_v,option)
  ...
  if (isubr == 1 .or. isubr == 6) then
    c_y = 1.d0 - s_a(1) * (T_r ** 2) + L_coeff(1) * (T_r ** 2)
    c_z = c_y + sqrt((s_a(3) * (c_y**2)) - (2.d0 * s_a(4) * T_r) + &
          (2.d0 * s_a(5) * P_r))
    r_v = c_a(12) * s_a(5) * (c_z **(-0.5d0/17.d0)) + (c_a(13) + c_a(14) * &
    ...
    r_v = r_v + 3.d0 * c_a(22) * (s_a(12) - T_r) * (P_r **2) + &
          4.d0 * c_a(23) * c_a(23) * (P_r**3) / (T_r **20)
...
subroutine SCO2VaporDiffusionCoeff(T,P,diffusion_coeff)
  !
  ! Computes CO2-Water diffusion coefficient, Reid et al., 1987
  !
  ! Author: Michael Nole
  ! Date: 12/04/23
  !
  ...
  PetscReal, intent(in) :: T
  PetscReal, intent(in) :: P
  PetscReal, intent(out) :: diffusion_coeff
  ...
  diffusion_coeff = (3.03d0 - (9.8d-1 / sqrt(w_mix))) * 1.d-3 * &
                    (T_k ** 1.5d0) / (P_bar * sqrt(w_mix) * (sig ** 2) * &
                     omega) * 1.d-4

end subroutine SCO2VaporDiffusionCoeff
...
subroutine SCO2ComputeSatHysteresis(characteristic_curves, Pc, Sl_min, Sl, &
                                    Sgt, option)
  !
  ...
  call characteristic_curves%saturation_function%Saturation(Pc,Sl,dsat_dpres, &
                                                            option)
...
subroutine SCO2ComputePcHysteresis(characteristic_curves, Sl, Sgt, Pc, option)
  !
  ...
  select case(sco2_diffusion_model)
    ...
    case(ONE_INTEGER)
      ...
      sco2_auxvar%effective_diffusion_coeff(sid,lid) = &
                ...
                (sco2_auxvar%temp + 273.15d0) / SALT_REFERENCE_TEMPERATURE * &
