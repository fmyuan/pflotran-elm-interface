...
subroutine DenitrSetup(this,reaction,option)
  ...
  word = 'NO3-'
  this%ispec_no3 = ReactionAuxGetPriSpecIDFromName(word,reaction, option)
  ...
  word = 'N2O(aq)'
  this%ispec_n2o = ReactionAuxGetPriSpecIDFromName(word,reaction, option)
  ...
  word = 'N2(aq)'
  this%ispec_n2 = ReactionAuxGetPriSpecIDFromName(word,reaction, option)
  ...
  word = 'NGASdeni'
  this%ispec_ngasdeni = ReactionImGetSpeciesIDFromName(word,reaction%immobile, &
     PETSC_FALSE,option)
...
! denitrification (Dickinson et al. 2002)
  ...
  if( (option%print_file_flag) .and. &
    ...
    write(option%fid_out, *) 'dt=',option%tran_dt'
