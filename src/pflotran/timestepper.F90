...
#ifdef SURFACE_FLOW
  if (realization%option%nsurfflowdof > 0) then
...
#ifdef SURFACE_FLOW
    use Surface_Realization_class
    use Output_Surface_module, only : OutputSurface
#endif
...
#include "finclude/petscviewer.h"
  ...
  if (output_option%plot_number == 0 .and. &
    ...
    plot_flag_surf = PETSC_TRUE
    transient_plot_flag_surf = PETSC_TRUE
    call OutputSurface(surf_realization,realization,plot_flag_surf,transient_plot_flag_surf)
#endif
  endif
...
#include "finclude/petscviewer.h"
    ...
    step_to_steady_state = PETSC_FALSE
    call PetscTime(stepper_start_time, ierr)
...
!    endif
    ...
    !if (.not.associated(master_stepper%cur_waypoint) .or. stop_flag) exit
     ! if at end of waypoint list (i.e. cur_waypoint = null), we are done!
     if(pause_Time>0 .and. &
        flow_stepper%target_time/realization%output_option%tconv == pause_time) then
        exit
     endif
