...
function PressureCreate()
  ...
  PressureCreate%rate_constant = 0.d0
    ...
    select case(trim(word))
      ...
      case('PRESSURE')
        ...
        do i = 1, option%nphase
          ...
          select case(i)
            case(ONE_INTEGER)
              word = 'Liquid Phase Pressure'
            case(TWO_INTEGER)
              word = 'Gas Phase Pressure'
            case default
              write(word,*) i
              option%io_buffer = 'Unknown Phase' // StringWrite(i) // &
                                 ' in SrcSinkPressureRead.'
              call PrintErrMsg(option)
          end select
          call InputReadDouble(input,option,this%pressure(i))
          call InputErrorMsg(input,option,word,'SOURCE_SINK_SANDBOX,PRESSURE,')
          call InputReadAndConvertUnits(input,this%pressure(i),internal_units, &
                                  'SOURCE_SINK_SANDBOX,PRESSURE,'//trim(word), &
                                        option)
        enddo
      case default
...
subroutine PressureSrcSink(this,Residual,Jacobian,compute_derivative, &
  ...
  k = this%rate_constant
  do iphase = 1, option%nphase
    Residual(iphase) = -k*(this%pressure(iphase) - aux_real(iphase))
  enddo
  print *, 'pumping rate: ' // &
    StringWrite(Residual(1)/aux_real(SS_PRES_LIQUID_DENSITY_OFFSET)*3600.)

  if (compute_derivative) then
    Jacobian(iphase,iphase) = k
!    print *, 'Jac_ana:', k
  endif
