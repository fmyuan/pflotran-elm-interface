...
subroutine SubsurfaceSetFlowMode(pm_flow,option)
  ...
  if (.not.associated(pm_flow)) then
    ...
    option%nfluid    = 1
    option_flow%nspecliq  = 1  ! liq. water only
  ...
  select type(pm_flow)
    class is (pm_mpflow_type)
      ...
      option%nfluid    = 2     ! max. of idx of above 3 fluids
      option_flow%nspecliq  = 1     ! 0: no liq flow; 1: liq. water/solution fluid itself; 2-n: ?
      option_flow%nspecgas  = 0     ! 0: no air/gas flow; 1: air mixture; 2-n: multiple gases
      option_flow%nspecoil  = 0     ! 0: no oil flow; 1: oil-like fluid itself; 2-n: ?
      option_flow%nspecflow = 1     ! sum

      option%store_fluxes = PETSC_TRUE
    class default
      option%io_buffer = ''
      call PrintErrMsg(option)
...
#include "petsc/finclude/petscsnes.h"
  ...
  do
    ...
    select type(cur_pm)
      class is(pm_subsurface_flow_type)
        pmc%timestepper%dt = option_flow%dt
      ...
      case ('TIMESTEPPER')
        ...
        select case(word)
          case('FLOW')
            ...
            if (option%resdef) then
              option_flow%flowTimestepperDone = PETSC_TRUE
            endif
          case default
      ...
      case ('LINEAR_SOLVER')
        ...
        select case(word)
          case('FLOW')
            ...
            if (option%resdef) then
              option_flow%flowSolverLinearDone = PETSC_TRUE
            endif
          case default
...
!....................
      case ('END_SUBSURFACE')
        ...
        if (option%resdef) then
          ...
          if (.NOT. option_flow%flowSolverLinearDone) then
              call SolverReadLinear(flow_timestepper%solver,input,option)
              option_flow%flowSolverLinearDone = PETSC_TRUE
          endif
          if (.NOT. option_flow%flowTimestepperDone) then
            call flow_timestepper%ReadInput(input,option)
            option_flow%flowTimestepperDone = PETSC_TRUE
          endif
        endif
