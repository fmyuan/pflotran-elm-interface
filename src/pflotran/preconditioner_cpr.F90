...
subroutine CPRApply(p, r, y,ierr)
  ...
  if (ctx%CPR_type == "ADDITIVE") then
    ...
    call PCApply(ctx%T3_PC,r,t3r,ierr);CHKERRQ(ierr)
    call VecAYPX(t1r,one,t3r,ierr);CHKERRQ(ierr)
  end if
  ...
  call VecAYPX(r2,-one,r,ierr);CHKERRQ(ierr)
...
subroutine CPRSetupT1(ctx,  ierr)
  ...
  if (ctx%firstT1Call) then ! some final initializations that need knowlege
    ...
    call MatGetMaxRowCount(A,mx,ierr);CHKERRQ(ierr)
...
subroutine CPRSetupT2(ctx, ierr)
  ...
  T2 = ctx%T2_PC
    ...
    if (StringCompare(ctx%T2_type, 'PCASM')) then
      ...
      ! call again:
      call PCASMGetSubKSP(T2,nsub_ksp,first_sub_ksp,sub_ksps, &
                          ierr);CHKERRQ(ierr)
    ...
    elseif (StringCompare(ctx%T2_type, 'PCBJACOBI')) then
      ...
      allocate(sub_ksps(nsub_ksp))
      ! call again:
      call PCBJacobiGetSubKSP(T2,nsub_ksp,first_sub_ksp,sub_ksps, &
                              ierr);CHKERRQ(ierr)
...
subroutine MatGetSubQIMPES(a, ap, factors1Vec,  ierr, ctx)
  ...
  call MPI_Comm_Rank(PETSC_COMM_WORLD,rnk,ierr);CHKERRQ(ierr)
...
subroutine MatGetSubQIMPES_var(a, ap, factors1Vec,  ierr, &
  ...
  do i = 0,nblks_l-1
    ...
    do j = 0,b-1
       call VecSetValue(factors1Vec,firstRow+j,local_factors(j),INSERT_VALUES, &
                        ierr);CHKERRQ(ierr)
    end do
