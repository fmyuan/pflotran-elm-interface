...
subroutine HydrateCompositeThermalCond(phi,sat,kdry,kwet,keff)
  ...
  !keff = kdry*(1.d0-phi) + phi * (sat(lid)*k_h20 + sat(hid)*k_hyd + sat(iid)*k_ice &
  !       + sat(gid)*k_ch4)
...
subroutine HydratePE(T, sat, PE, dP, characteristic_curves, material_auxvar, &
  ...
  call HydrateSalinityOffset(hydrate_xmol_nacl,dTfs,option)
  dTf = dTf - dTfs
    ...
    select case(hydrate_phase_boundary)
      ...
      case(2)
        ...
        PE = exp(-1.9413850446456d5 + 3.31018213397926d3 * T_temp &
             ...
             T_temp**5)
        if (hydrate_adjust_ghsz_solubility .and. hydrate_with_gibbs_thomson) then
...
subroutine HydrateSalinityOffset(xmol,dTd,option)
  !
