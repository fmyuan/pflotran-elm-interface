...
!   CLM-CN
    case(MOISTURE_RESPONSE_FUNCTION_CLM4) 
      call VecGetArrayReadF90(clm_pf_idata%sucsat_sub_pfs, sucsat_pf_loc, ierr)
      CHKERRQ(ierr)
      call VecGetArrayReadF90(clm_pf_idata%bulkdensity_dry_sub_pfs, bd_dry_pf_loc, ierr)   ! 'bd' (kg/m3)
      CHKERRQ(ierr)
      call VecGetArrayReadF90(clm_pf_idata%bsw_sub_pfs, bsw_pf_loc, ierr)
      CHKERRQ(ierr)
      ...
      call VecRestoreArrayReadF90(clm_pf_idata%sucsat_sub_pfs, sucsat_pf_loc, ierr)
      CHKERRQ(ierr)
      call VecRestoreArrayReadF90(clm_pf_idata%bulkdensity_dry_sub_pfs, bd_dry_pf_loc, ierr)
      CHKERRQ(ierr)
      call VecRestoreArrayReadF90(clm_pf_idata%bsw_sub_pfs, bsw_pf_loc, ierr)
      CHKERRQ(ierr)

! DLEM 
! Tian et al. 2010 Biogeosciences, 7, 2673-2694 Eq. 13
    case(MOISTURE_RESPONSE_FUNCTION_DLEM) 
      call VecGetArrayReadF90(clm_pf_idata%effporosity_sub_pfs, porosity_pf_loc, ierr)
      CHKERRQ(ierr)
      call VecGetArrayReadF90(clm_pf_idata%watfc_sub_pfs, watfc_pf_loc, ierr)
      CHKERRQ(ierr)
      ...
      call VecRestoreArrayReadF90(clm_pf_idata%effporosity_sub_pfs, porosity_pf_loc, ierr)
      CHKERRQ(ierr)
      call VecRestoreArrayReadF90(clm_pf_idata%watfc_sub_pfs, watfc_pf_loc, ierr)
      CHKERRQ(ierr)
    case default
        F_theta = 1.0d0
  end select
