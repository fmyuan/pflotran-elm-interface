...
#include "petsc/finclude/petscsys.h"
  ...
  ! 2 dofs currently
  PetscInt, public :: IFDOF1 = FLOW_TEMPERATURE_DOF
  PetscInt, public :: IFDOF2 = FLOW_LIQ_PRESSURE_DOF   ! by default it's for liq. water
  ...
  type, public, extends(auxvar_flow_energy_type) :: flow_auxvar_type
    ...
    PetscReal, pointer :: kvr(:)          ! (nfluids)
    PetscReal, pointer :: dkvr(:,:)       ! (nfluids, nflowdof)

    ! Effective thermal conductivity (all materials, ice/water/vapor+soil particle?)
    PetscReal :: Dk_eff
...
#include "petsc/finclude/petscsys.h"
  ...
  allocate(auxvar%dmolv_air(option%nflowdof))
  ...
  allocate(auxvar%kvr(option%nfluids))
  auxvar%kvr       = UNINITIALIZED_DOUBLE
  allocate(auxvar%dkvr(option%nfluids,option%nflowdof))
...
!***************  3-phase water properties **********************************************************
  ...
  auxvar%viscosity(option%liq_fluid) = visl
  auxvar%kvr(option%liq_fluid) = kr/visl
  auxvar%dkvr(option%liq_fluid,IFDOF1) = dkr_dp/visl - kr/(visl*visl)*dvis_dp
  auxvar%dkvr(option%liq_fluid,IFDOF2) = -kr/(visl*visl)*dvis_dt + dkr_dt/visl
...
subroutine FlowmodeAuxVarComputeCharacteristicCurves( presl,  tc,           &
  ...
  if (option%th_ice_model /= UNINITIALIZED_INTEGER) then
