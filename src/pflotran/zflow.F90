...
subroutine ZFlowSetup(realization)
  ...
  if (zflow_use_lumped_harm_flux) then
    XXFlux => ZFlowFluxLumpedHarmonic
    XXBCFlux => ZFlowBCFluxLumpedHarmonic
  else
...
subroutine ZFlowInitializeTimestep(realization)
  ...
  zflow_newton_iteration_number = 0
...
subroutine ZFlowUpdateSolution(realization)
  ...
  call ZFlowCreepShutDown(realization)
...
subroutine ZFlowComputeMassBalance(realization,mass_balance)
  ...
  do local_id = 1, grid%nlmax
    ...
    do iphase = 1, option%nphase
      ...
      vol_phase = &
        zflow_auxvars(ZERO_INTEGER,ghosted_id)%sat(iphase)* &
      ...
      mass_balance(iphase) = mass_balance(iphase) + &
        zflow_auxvars(ZERO_INTEGER,ghosted_id)%den(iphase)* &
...
subroutine ZFlowUpdateAuxVars(realization)
  ...
  do 
    ...
    do iconn = 1, cur_connection_set%num_connections
      ...
      istate = boundary_condition%flow_aux_int_var(ZFLOW_STATE_INDEX,iconn)
          ...
          select case(boundary_condition%flow_bc_type(idof))
            ...
            case(DIRICHLET_BC)
              ...
              select case(variable)
                ...
                case(ZFLOW_LIQUID_PRESSURE_INDEX)
                ...
                case(ZFLOW_GAS_SATURATION_INDEX)
                  real_index = boundary_condition%flow_aux_mapping(variable)
                  if (real_index /= 0) then
                    xxbc(idof) = boundary_condition% &
                                   flow_aux_real_var(real_index,iconn)
            ...
            case(NEUMANN_BC)
            case default
              option%io_buffer = 'Unknown BC type in ZFlowUpdateAuxVars().'
              call PrintErrMsg(option)
          end select
        enddo  
      else
...
subroutine ZFlowUpdateFixedAccum(realization)
  ...
  do local_id = 1, grid%nlmax
    ...
    call ZFlowAccumulation(zflow_auxvars(ZERO_INTEGER,ghosted_id), &
                             ...
                             PETSC_FALSE)
    call ZFlowConvertUnitsToBRAGFlo(accum_p(local_start:local_end), &
...
subroutine ZFlowResidual(snes,xx,r,realization,ierr)
  ...
  if (.not.zflow_jacobian_test_active) then
    zflow_newton_iteration_number = zflow_newton_iteration_number + 1
    ! bragflo uses the following logic, update when
    !   it == 1, before entering iteration loop
    !   it > 1 and mod(it-1,frequency) == 0
    ! the first is set in ZFlowInitializeTimestep, the second is set here
    if (zflow_newton_iteration_number > 1 .and. &
        mod(zflow_newton_iteration_number-1, &
  ...
  do local_id = 1, grid%nlmax  ! For each local node do...
    ...
    if (zflow_residual_test .and. &
      ...
      write(*,'(" A(calc): ",i5,8es12.4)') &
        zflow_residual_test_cell, &
        zflow_auxvars(ZERO_INTEGER,ghosted_id)%effective_porosity, &
        zflow_auxvars(ZERO_INTEGER,ghosted_id)%den_kg(1), &
        zflow_auxvars(ZERO_INTEGER,ghosted_id)%sat(1), &
        zflow_auxvars(ZERO_INTEGER,ghosted_id)%den_kg(2), &
        zflow_auxvars(ZERO_INTEGER,ghosted_id)%sat(2)
      write(*,'(" A: ",2es12.4,i5)') &
  ...
  if (field%flow_mass_transfer /= PETSC_NULL_VEC) then
    ...
    do local_id = 1, grid%nlmax  ! For each local node do...
      ...
      call ZFlowConvertUnitsToBRAGFlo(Res,material_auxvars(ghosted_id),option)
      r_p(local_start:local_end) = r_p(local_start:local_end) - Res
    enddo
    call VecRestoreArrayF90(r, r_p, ierr);CHKERRQ(ierr)
...
subroutine ZFlowJacobian(snes,xx,A,B,realization,ierr)
  ...
  do local_id = 1, grid%nlmax  ! For each local node do...
    ...
    call ZFlowAccumDerivative(zflow_auxvars(:,ghosted_id), &
                              ...
                              option, &
                              Jup) 
    call ZFlowConvertUnitsToBRAGFlo(Jup,material_auxvars(ghosted_id), &
                                      option)
  ...
  do 
    ...
    do iconn = 1, cur_connection_set%num_connections
      ...
      if (local_id_up > 0) then
        Jtmp = Jup
        call ZFlowConvertUnitsToBRAGFlo(Jtmp, &
                                          material_auxvars(ghosted_id_up), &
                                          option)
  ...
  do 
    ...
    do iconn = 1, cur_connection_set%num_connections
      ...
      Jdn = -Jdn
      call ZFlowConvertUnitsToBRAGFlo(Jdn,material_auxvars(ghosted_id), &
                                        option)
  ...
  do 
    ...
    do iconn = 1, cur_connection_set%num_connections
      ...
      global_auxvars_bc(sum_connection)%den_kg = &
        zflow_auxvars_bc(sum_connection)%den_kg
    enddo
    boundary_condition => boundary_condition%next
  enddo
