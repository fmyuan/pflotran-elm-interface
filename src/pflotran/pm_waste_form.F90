...
subroutine PMWFReadMechanism(this,input,option,keyword,error_string,found)
  ...
  !
  use Input_Aux_module
  use Reaction_Aux_module, only: ReactionGetPriSpeciesIDFromName
  use Option_module
    ...
    select type(cur_mechanism)
      ...
      type is(wf_mechanism_fmdm_type)
        ...
        cur_mechanism%mapping_fmdm_to_pflotran(cur_mechanism%iO2) = &
          ReactionGetPriSpeciesIDFromName(species_name,reaction,option)
        species_name = 'CO3--'
      ...
      type is(wf_mechanism_glass_type)
        ...
        if (cur_mechanism%use_Q) then
          ...
          if (associated(this%realization%reaction)) then
            ...
            ! message from ReactionGetPriSpeciesIDFromName is not thrown first
            ! when SiO2 is missing
            allocate(names(ReactionGetPriSpeciesCount( &
                             this%realization%reaction)))
            names => ReactionGetPriSpeciesNames(this%realization%reaction)
            i = 0
            found = PETSC_FALSE
            do while (i < len(names))
              ...
              if (adjustl(trim(species_name)) == adjustl(trim(names(i)))) then
                cur_mechanism%SiO2_id = &
                  ReactionGetPriSpeciesIDFromName(species_name, &
                                                  this%realization%reaction, &
                                                  option)
                found = PETSC_TRUE
              endif
              ...
              if ((.not.found) .and. (i == len(names))) then
                deallocate(names)
                allocate(names(ReactionGetSecSpeciesCount( &
                                 this%realization%reaction)))
                names => ReactionGetSecSpeciesNames(this%realization%reaction)
                i = 0
                do while (i < len(names))
                  i = i + 1
                  if (adjustl(trim(species_name)) == &
                    ...
                    cur_mechanism%SiO2_id = &
                      ReactionGetSecSpeciesIDFromName(species_name, &
