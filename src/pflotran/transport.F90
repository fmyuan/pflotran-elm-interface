...
#include "petsc/finclude/petscsnes.h"
  ...
  public :: TranTimeCut, &
            TranSetup, &
            TranMaxChange, &
            TranUpdateEquilibriumState, &
            TranUpdateKineticState, &
            TranUpdateMassBalance, &
            TranResidual, &
            TranJacobian, &
            TranInitializeTimestep, &
            TranUpdateAuxVars, &
            TranComputeMassBalance, &
            TranDestroy, &
            TranUpdateTransportCoefs, &
            TranUpdateActivityCoefficients, &
            TranUpdateRHSCoefs, &
            TranCalculateRHS_t0, &
            TranCalculateRHS_t1, &
            TranCalculateTransportMatrix, &
            TranReact, &
            TranJumpStartKineticSorption, &
            TranCheckpointKineticSorptionBinary, &
            TranCheckpointKineticSorptionHDF5, &
            TranExplicitAdvection, &
            TranClearActivityCoefficients
  
contains

! ************************************************************************** !

subroutine TranTimeCut(realization)
  ! 
  ...
  use Field_module
  use Global_module
  use Secondary_Continuum_module, only : SecondaryTranTimeCut
  ...
  if (option%use_mc) then
    call SecondaryTranTimeCut(realization)
  endif
 
end subroutine TranTimeCut

! ************************************************************************** !

subroutine TranSetup(realization)
  ! 
  ! Author: Glenn Hammond
  ! Date: 02/22/08
  ! 
  ...
  use Secondary_Continuum_Aux_module, only : sec_transport_type, &
                                             SecondaryAuxTranCreate
  use Secondary_Continuum_module, only : SecondaryTranAuxVarInit
  ...
  sec_tran_constraint => &
    TranConstraintCast(realization%sec_transport_constraint)

  patch%aux%Tran => TranAuxCreate(reaction%naqcomp,option%transport%nphase)
  rt_parameter => patch%aux%Tran%rt_parameter
  ! rt_parameter %naqcomp and %nphase set in TranAuxCreate()
  ...
  if (maxval(flag) > 0) then
    option%io_buffer = &
      'Material property errors found in TranSetup (reactive transport).'
...
!============== Create secondary continuum variables - SK 2/5/13 ===============
  ...
  if (option%use_mc) then
    patch%aux%SC_Tran => SecondaryAuxTranCreate(option)
    ...
    ! Assuming the same secondary continuum type for all regions
      call SecondaryTranAuxVarInit(patch%material_property_array(1)%ptr, &
                                 rt_sec_transport_vars(local_id), &
                                 reaction,initial_condition, &
                                 sec_tran_constraint,option)
    enddo      
    patch%aux%SC_Tran%sec_transport_vars => rt_sec_transport_vars
  endif
...
!===============================================================================   
...
#endif
  allocate(patch%aux%Tran%auxvars(grid%ngmax))
  do ghosted_id = 1, grid%ngmax
    call TranAuxVarInit(patch%aux%Tran%auxvars(ghosted_id),reaction,option)
  enddo
  patch%aux%Tran%num_aux = grid%ngmax
  ...
  if (sum_connection > 0) then
    ...
    allocate(patch%aux%Tran%auxvars_bc(sum_connection))
    do iconn = 1, sum_connection
      call TranAuxVarInit(patch%aux%Tran%auxvars_bc(iconn),reaction,option)
    enddo
  endif
  patch%aux%Tran%num_aux_bc = sum_connection
  ...
  if (sum_connection > 0) then
    option%iflag = 1 ! enable allocation of mass_balance array 
    allocate(patch%aux%Tran%auxvars_ss(sum_connection))
    do iconn = 1, sum_connection
      call TranAuxVarInit(patch%aux%Tran%auxvars_ss(iconn),reaction,option)
    enddo
  endif
  patch%aux%Tran%num_aux_ss = sum_connection
  option%iflag = 0
  ...
  call TranSetPlotVariables(list,reaction,option, &
...
end subroutine TranSetup

! ************************************************************************** !

subroutine TranComputeMassBalance(realization,max_size,sum_mol)
  ! 
  ! Author: Glenn Hammond
  ! Date: 12/23/08
  ! 
  ...
  rt_auxvars => patch%aux%Tran%auxvars
...
end subroutine TranComputeMassBalance

! ************************************************************************** !

subroutine TranZeroMassBalanceDelta(realization)
  ! 
  ...
  rt_auxvars_bc => patch%aux%Tran%auxvars_bc
  rt_auxvars_ss => patch%aux%Tran%auxvars_ss

#ifdef COMPUTE_INTERNAL_MASS_FLUX
  do iconn = 1, patch%aux%Tran%num_aux
    patch%aux%Tran%auxvars(iconn)%mass_balance_delta = 0.d0
  enddo
#endif

  do iconn = 1, patch%aux%Tran%num_aux_bc
    rt_auxvars_bc(iconn)%mass_balance_delta = 0.d0
  enddo

  do iconn = 1, patch%aux%Tran%num_aux_ss
    rt_auxvars_ss(iconn)%mass_balance_delta = 0.d0
  enddo

end subroutine TranZeroMassBalanceDelta

! ************************************************************************** !

subroutine TranUpdateMassBalance(realization)
  ! 
...
#ifdef COMPUTE_INTERNAL_MASS_FLUX
  do iconn = 1, patch%aux%Tran%num_aux
    patch%aux%Tran%auxvars(iconn)%mass_balance = &
      patch%aux%Tran%auxvars(iconn)%mass_balance + &
      patch%aux%Tran%auxvars(iconn)%mass_balance_delta*option%tran_dt
  enddo
#endif
...
end subroutine TranUpdateMassBalance

! ************************************************************************** !

subroutine TranInitializeTimestep(realization)
  ! 
  ! Author: Glenn Hammond
  ! Date: 02/22/08
  ! 
  ...
  call VecCopy(realization%field%tran_xx,realization%field%tran_yy, &
               ierr);CHKERRQ(ierr)
  call TranUpdateFixedAccumulation(realization)

end subroutine TranInitializeTimestep

! ************************************************************************** !

subroutine TranUpdateEquilibriumState(realization)
  ! 
  ...
  !      resolves a bug in the Intel Visual Fortran compiler.
  use Secondary_Continuum_Aux_module, only : sec_transport_type
  use Secondary_Continuum_module, only : SecondaryTranUpdateEquilState
  ...
  call TranUpdateAuxVars(realization,PETSC_TRUE,PETSC_FALSE,PETSC_FALSE)
...
!geh: for debugging max/min concentrations
  ...
  if (option%use_mc) then
    rt_sec_transport_vars => patch%aux%SC_Tran%sec_transport_vars
    do local_id = 1, grid%nlmax
      ghosted_id = grid%nL2G(local_id)
      if (patch%imat(ghosted_id) <= 0) cycle
        call SecondaryTranUpdateEquilState(rt_sec_transport_vars(local_id), &
                                          global_auxvars(ghosted_id), &
                                          reaction,option)                     
    enddo
  endif
  
end subroutine TranUpdateEquilibriumState

! ************************************************************************** !

subroutine TranUpdateKineticState(realization)
  ! 
  ...
  !      resolves a bug in the Intel Visual Fortran compiler.
  use Secondary_Continuum_Aux_module, only : sec_transport_type
  use Secondary_Continuum_module, only : SecondaryTranUpdateKineticState
  ...
  if (option%use_mc) then
    ...
    do local_id = 1, grid%nlmax
      ...
      if (patch%imat(ghosted_id) <= 0) cycle
        ...
        call SecondaryTranUpdateKineticState(rt_sec_transport_vars(local_id), &
                                           global_auxvars(ghosted_id), &
                                           reaction,sec_porosity,option)                     
    enddo
  endif
  
end subroutine TranUpdateKineticState

! ************************************************************************** !

subroutine TranUpdateFixedAccumulation(realization)
  ! 
...
! Do not use TranUpdateAuxVars() as it loops over ghosted ids
  ...
  do local_id = 1, grid%nlmax
    ...
    ! FIXED POTranION OF THE ACCUMULATION TERM - geh
    call TranAuxVarCompute(rt_auxvars(ghosted_id), &
                         global_auxvars(ghosted_id), &
                         material_auxvars(ghosted_id), &
                         reaction,option)
    call TranAccumulation(rt_auxvars(ghosted_id), &
                        global_auxvars(ghosted_id), &
                        material_auxvars(ghosted_id), &
                        reaction,option, &
...
end subroutine TranUpdateFixedAccumulation

! ************************************************************************** !

subroutine TranUpdateTransportCoefs(realization)
  ! 
...
end subroutine TranUpdateTransportCoefs

! ************************************************************************** !

subroutine TranUpdateRHSCoefs(realization)
  ! 
...
end subroutine TranUpdateRHSCoefs

! ************************************************************************** !

subroutine TranCalculateRHS_t0(realization)
  ! 
...
end subroutine TranCalculateRHS_t0

! ************************************************************************** !

subroutine TranCalculateRHS_t1(realization)
  ! 
  ! Calculate porition of RHS of transport system
  ! at time level k+1
  ! 
