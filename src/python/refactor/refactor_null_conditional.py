...
#regex_string = r'(?P<pre>.*if\s*\()\s*(?P<variable>\S*)\s*' + \
#regex_string = r'(?P<pre>.*\()\s*(?P<variable>\S*)\s*' + \
regex_string = r'(?P<pre>.*[\s+(])(?P<variable>\S+)\s*' + \
               r'(?P<condition>[=/]=)\s*' + \
...
def refactor(filename,overwrite):
    ...
    for line in fin:
        ...
        if m:
            if m.group('condition').startswith('='):
                routine = 'PetscObjectIsNull('
            else:
                routine = '.not.PetscObjectIsNull('
