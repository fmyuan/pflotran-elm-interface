module pflotran_model_main_module
...
#include "finclude/petscvec.h"
  ...
  public::pflotranModelCreate,               &
       ...
       ! soil properties
       pflotranModelSetSoilPropFromCLM,      &
       pflotranModelGetSoilPropFromPF,       &
       pflotranModelResetSoilPoroFromCLM,    &
       ! T/H
       pflotranModelSetTHstatefromCLM,       &    ! dynamically update TH from CLM to PF's global vars (flow mode is off) to drive PFLOTRAN BGC
       pflotranModelSetInternalTFromCLM,     &    ! T from CLM to PFLOTRAN when flow-only mode is on
       pflotranModelUpdateFlowCondsFromCLM,  &
       pflotranModelSetSoilTbcsFromCLM,      &
       pflotranModelSetSoilHbcsFromCLM,      &
       pflotranModelGetTStatesFromPF,        &
       pflotranModelGetHStatesFromPF,        &
       ! BGC
       pflotranModelSetBgcConcsFromCLM,      &
       pflotranModelSetBgcRatesFromCLM,      &
       pflotranModelUpdateAqConcsFromCLM,    &
       pflotranModelUpdateAqGasesFromCLM,    &
       pflotranModelGetBgcVariablesFromPF,   &
       ! THC mass-balance at BC from PF
       pflotranModelGetBCMassBalDeltaFromPF
...
!    use pflotran_clm_setmapping_module, only: pflotranModelSetupMappingFiles
...
#include "finclude/petscsys.h"
    ...
    !call OptionInitMPI(model%option)   ! this is pflotran's MPI initialization (FMY: need to talk with GB)
...
!
! ************************************************************************** !
!  THE FOLLOWING CODE BLOCKS are FOR Thermal-Hydrology (TH) coupling
! ************************************************************************** !
!
  subroutine pflotranModelSetSoilPropFromCLM(pflotran_model)
  !
...
#include "finclude/petscvec.h90"
    ...
    PetscScalar, pointer :: zsoi_sub_pf_loc(:)               ! soil depth (m)
    PetscScalar, pointer :: bulkdensity_dry_sub_pf_loc(:)    ! bulk density (kg/m3)
    PetscScalar, pointer :: effporosity_sub_pf_loc(:)        ! effective porosity for liq. + air (m3/m3)
    ...
    PetscScalar, pointer :: watsat_sub_pf_loc(:)  ! volumetric soil water at saturation (porosity)
    PetscScalar, pointer :: watfc_sub_pf_loc(:)   ! volumetric soil water at field capacity
    ! Clap-Hornberger's
    PetscScalar, pointer :: sucsat_sub_pf_loc(:)  ! minimum soil suction (mm)
    PetscScalar, pointer :: bsw_sub_pf_loc(:)     ! Clapp and Hornberger "b"
    ! MVM's
    PetscScalar, pointer :: sr_sub_pf_loc(:)      ! volumetric soil water saturation at very dry condition (residue saturation)
    PetscScalar, pointer :: lamda_sub_pf_loc(:)      !
    PetscScalar, pointer :: alpha_sub_pf_loc(:)      !
    PetscScalar, pointer :: pcwmax_sub_pf_loc(:)     !
    ! thermal's
    PetscScalar, pointer :: tcond_dry_sub_pf_loc(:)  !  bulk thermal conductivity at dry condition
    PetscScalar, pointer :: tcond_wet_sub_pf_loc(:)  !  bulk thermal conductivity at saturated condition
    PetscScalar, pointer :: hc_dry_sub_pf_loc(:)     !  bulkd heat capacity at dry
    ...
    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    ...
                                    clm_pf_idata%zsoi_sub_clmp, &
                                    clm_pf_idata%zsoi_sub_pfs)

    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%bulkdensity_dry_sub_clmp, &
                                    clm_pf_idata%bulkdensity_dry_sub_pfs)

    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%effporosity_sub_clmp, &
                                    clm_pf_idata%effporosity_sub_pfs)
    ...
    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%watsat_sub_clmp, &
                                    clm_pf_idata%watsat_sub_pfs)

    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%watfc_sub_clmp, &
                                    clm_pf_idata%watfc_sub_pfs)

    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%sucsat_sub_clmp, &
                                    clm_pf_idata%sucsat_sub_pfs)
    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%bsw_sub_clmp, &
                                    clm_pf_idata%bsw_sub_pfs)

    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%sr_sub_clmp, &
                                    clm_pf_idata%sr_sub_pfs)
    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%lamda_sub_clmp, &
                                    clm_pf_idata%lamda_sub_pfs)
    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%alpha_sub_clmp, &
                                    clm_pf_idata%alpha_sub_pfs)
    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%pcwmax_sub_clmp, &
                                    clm_pf_idata%pcwmax_sub_pfs)

    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%tcond_dry_sub_clmp, &
                                    clm_pf_idata%tcond_dry_sub_pfs)
    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%tcond_wet_sub_clmp, &
                                    clm_pf_idata%tcond_wet_sub_pfs)
    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%hc_dry_sub_clmp, &
                                    clm_pf_idata%hc_dry_sub_pfs)


    !-------------------------------------------------------------------
    ! interface data vecs to array
    call VecGetArrayF90(clm_pf_idata%zsoi_sub_pfs, zsoi_sub_pf_loc, ierr)
    CHKERRQ(ierr)
    call VecGetArrayF90(clm_pf_idata%bulkdensity_dry_sub_pfs, bulkdensity_dry_sub_pf_loc, ierr)
    CHKERRQ(ierr)
    call VecGetArrayF90(clm_pf_idata%effporosity_sub_pfs, effporosity_sub_pf_loc, ierr)
    CHKERRQ(ierr)
    call VecGetArrayF90(clm_pf_idata%watsat_sub_pfs,  watsat_sub_pf_loc,  ierr)
    CHKERRQ(ierr)
    call VecGetArrayF90(clm_pf_idata%watfc_sub_pfs, watfc_sub_pf_loc, ierr)
    CHKERRQ(ierr)
    ...
    call VecGetArrayF90(clm_pf_idata%sucsat_sub_pfs, sucsat_pf_loc, ierr)
    CHKERRQ(ierr)
    call VecGetArrayF90(clm_pf_idata%bsw_sub_pfs, bsw_sub_pf_loc,ierr)
    CHKERRQ(ierr)

    call VecGetArrayF90(clm_pf_idata%sr_sub_pfs, sr_sub_pf_loc,  ierr)
    CHKERRQ(ierr)
    call VecGetArrayF90(clm_pf_idata%lamda_sub_pfs, lamda_sub_pf_loc,  ierr)
    CHKERRQ(ierr)
    call VecGetArrayF90(clm_pf_idata%alpha_sub_pfs, alpha_sub_pf_loc,  ierr)
    CHKERRQ(ierr)
    call VecGetArrayF90(clm_pf_idata%pcwmax_sub_pfs, pcwmax_sub_pf_loc,  ierr)
    CHKERRQ(ierr)

    call VecGetArrayF90(clm_pf_idata%tcond_dry_sub_pfs, tcond_dry_sub_pf_loc,  ierr)
    CHKERRQ(ierr)
    call VecGetArrayF90(clm_pf_idata%tcond_wet_sub_pfs, tcond_wet_sub_pf_loc,  ierr)
    CHKERRQ(ierr)
    call VecGetArrayF90(clm_pf_idata%hc_dry_sub_pfs, hc_dry_sub_pf_loc,  ierr)
    CHKERRQ(ierr)


    ! PF data vecs to array
    ...
    !(TODO) need a better way to generate MVG parameters from CLM inputs
      ...
      if(pflotran_model%option%nflowdof > 0) then
       ...
       ! currently VG-Mulaem saturation function type
        if ( saturation_function%saturation_function_itype == VAN_GENUCHTEN .and. &
          ...
          saturation_function%alpha  = alpha_pf_sub_loc(ghosted_id)
          saturation_function%lambda = lamda_pf_sub_loc(ghosted_id)   ! 'm' in VG function ( or, n=1/(1-m))
          saturation_function%Sr(1)  = sr_pf_sub_loc(ghosted_id)      ! currently only for liq. water, NOTE that if at 'Sr', pc = inf
          saturation_function%pcwmax = pcwmax_pf_sub_loc(ghosted_id)  ! this parameter IS not corresponding with 'Sr'

       ! currently BC-Burdine saturation function type, with specified values to match with Clapp-Hornberger Eq.
        elseif ( saturation_function%saturation_function_itype == BROOKS_COREY .and. &
          ...
          saturation_function%alpha  = -9.81d0*sucsat*pf_sub_loc(ghosted_id)
          saturation_function%lambda = 1.d0/bsw_pf_sub_loc(ghosted_id)   !
          saturation_function%Sr(1)  = 0.d0
          saturation_function%pcwmax = pcwmax_pf_sub_loc(ghosted_id)  ! this parameter IS not corresponding with 'Sr'

        else
            call printErrMsg(pflotran_model%option, &
               'Currently ONLY support VG-Mulaem or BC-Burdine saturation function type when coupled with CLM')
      ...
      ! initial porosity IS going to 'field%porosity0'
      porosity_loc_p(local_id) = watsat_sub_pf_loc(ghosted_id)

      ! material property passing
      material_property => patch%  &
           material_property_array(patch%imat(local_id))%ptr

      ! 'bulkdensity_dry' passing to material_property and 'patch%material_auxvars%soil_particle_density
      material_property%rock_density = bulkdensity_dry_sub_pf_loc(ghosted_id) &   ! kg/m3 bulk volume
                                       /(1.0d0-watsat_sub_pf_loc(ghosted_id))     ! ==> kg/m3 solid volume

      material_auxvars(local_id)%soil_particle_density = material_property%rock_density

      ! 'tcond_dry' and 'hc_dry' passing to 'patch%material_property_array(imat)' as 'saturation function array'
      ! OR, going to 'patch%aux%TH%TH_parameter', which are used directly in 'th.F90' and initialized from the global one.
      ! OR, maybe both (in case this subroutine called after 'th mode' initialization

      ! global realization
      material_property%specific_heat = hc_sub_pf_loc(ghosted_id)  &   ! specific heat: J/kg/K  <== heat capacity: J/m3/K  (dry/solid density)
             / material_auxvars(local_id)%soil_particle_density
      material_property%thermal_conductivity_dry = tcond_dry_sub_pf_loc(ghosted_id)    ! W/m/K
      material_property%thermal_conductivity_wet = tcond_wet_sub_pf_loc(ghosted_id)

      ! th_mode specific auxvars
      if (pflotran_model%option%iflowmode==TH_MODE) then
        th_parameter%dencpr(patch%imat(local_id)) = hc_sub_pf_loc(ghosted_id)
        th_parameter%ckdry(patch%imat(local_id))  = tcond_dry_sub_pf_loc(ghosted_id)
        th_parameter%ckwet(patch%imat(local_id))  = tcond_wet_sub_pf_loc(ghosted_id)

      end if

    enddo

    call VecRestoreArrayF90(clm_pf_idata%zsoi_sub_pfs, zsoi_sub_pf_loc, ierr)
    CHKERRQ(ierr)
    call VecRestoreArrayF90(clm_pf_idata%bulkdensity_dry_sub_pfs, bulkdensity_dry_sub_pf_loc, ierr)
    CHKERRQ(ierr)
    call VecRestoreArrayF90(clm_pf_idata%effporosity_sub_pfs, effporosity_sub_pf_loc, ierr)
    CHKERRQ(ierr)
    call VecRestoreArrayF90(clm_pf_idata%watsat_sub_pfs,  watsat_sub_pf_loc,  ierr)
    CHKERRQ(ierr)
    call VecRestoreArrayF90(clm_pf_idata%watfc_sub_pfs, watfc_sub_pf_loc, ierr)
    CHKERRQ(ierr)
    ...
    call VecRestoreArrayF90(clm_pf_idata%sucsat_sub_pfs, sucsat_pf_loc, ierr)
    CHKERRQ(ierr)
    call VecRestoreArrayF90(clm_pf_idata%bsw_sub_pfs, bsw_sub_pf_loc,ierr)
    CHKERRQ(ierr)

    call VecRestoreArrayF90(clm_pf_idata%sr_sub_pfs, sr_sub_pf_loc,  ierr)
    CHKERRQ(ierr)
    call VecRestoreArrayF90(clm_pf_idata%lamda_sub_pfs, lamda_sub_pf_loc,  ierr)
    CHKERRQ(ierr)
    call VecRestoreArrayF90(clm_pf_idata%alpha_sub_pfs, alpha_sub_pf_loc,  ierr)
    CHKERRQ(ierr)
    call VecRestoreArrayF90(clm_pf_idata%pcwmax_sub_pfs, pcwmax_sub_pf_loc,  ierr)
    CHKERRQ(ierr)

    call VecRestoreArrayF90(clm_pf_idata%tcond_dry_sub_pfs, tcond_dry_sub_pf_loc,  ierr)
    CHKERRQ(ierr)
    call VecRestoreArrayF90(clm_pf_idata%tcond_wet_sub_pfs, tcond_wet_sub_pf_loc,  ierr)
    CHKERRQ(ierr)
    call VecRestoreArrayF90(clm_pf_idata%hc_dry_sub_pfs, hc_dry_sub_pf_loc,  ierr)
    CHKERRQ(ierr)
  ...
  end subroutine pflotranModelSetSoilPropFromCLM
...
#include "finclude/petscvec.h90"
    ...
    PetscScalar, pointer :: effporosity_pf_loc(:)  ! these are from 'clm-pf-idata%'
    ...
    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    ...
                                    clm_pf_idata%effporosity_sub_clmp, &
                                    clm_pf_idata%effporosity_sub_pfs)
    ! NO need to map other parameters

    ! for adjusting porosity
    call VecGetArrayF90(clm_pf_idata%effporosity_sub_pfs, effporosity_pf_loc,  ierr)
    ...
    if (pflotran_model%option%nflowdof > 0) then
        ...
        call VecGetArrayF90(clm_pf_idata%effporosity_sub_pfs, effporosity_pf_loc, ierr)
        CHKERRQ(ierr)
        call VecGetArrayF90(clm_pf_idata%bsw_sub_pfs,  bsw_pf_loc,  ierr)
        CHKERRQ(ierr)
    ...
    do ghosted_id = 1, grid%ngmax
      ...
      if (pflotran_model%option%nflowdof > 0) then
           ...
           tempreal = effporosity_pf_loc(ghosted_id)/watsat_pf_loc(ghosted_id)
    ...
    call VecRestoreArrayF90(clm_pf_idata%effporosity_sub_pfs,  effporosity_pf_loc,  ierr)
    CHKERRQ(ierr)
    ...
    if (pflotran_model%option%nflowdof > 0) then
        ...
        call VecRestoreArrayF90(clm_pf_idata%effporosity_sub_pfs, effporosity_pf_loc,  ierr)
        CHKERRQ(ierr)
        call VecRestoreArrayF90(clm_pf_idata%bsw_sub_pfs,  bsw_pf_loc,  ierr)
        CHKERRQ(ierr)
...
#include "finclude/petscvec.h90"
    ...
    PetscScalar, pointer :: watsat_loc_pfp(:)      ! soil porosity
    PetscScalar, pointer :: watpcwmax_loc_pfp(:)   ! soil vwc at max. capillary pressure (note: not 'Sr')
    ...
    call VecGetArrayF90(clm_pf_idata%watsat_sub_pfp, watsat_loc_pfp, ierr)
    CHKERRQ(ierr)
    call VecGetArrayF90(clm_pf_idata%watpcwmax_sub_pfp, watpcwmax_loc_pfp, ierr)
    CHKERRQ(ierr)
    call VecGetArrayF90(clm_pf_idata%pcwmax_sub_pfp, pcwmax_loc_pfp, ierr)
    CHKERRQ(ierr)
    ...
    do local_id=1,grid%nlmax
        ...
        ! PF's porosity (initial, not transient)
        watsat_loc_pfp(local_id) = porosity_loc_p(local_id)
        ...
        call SaturationFunctionCompute(saturation_function%pcwmax, tempreal, &
                                      saturation_function, &
                                      pflotran_model%option)
        watpcwmax_loc_pfp(local_id) = tempreal*porosity_loc_p(local_id)   ! saturation --> vwc

    enddo

    call VecRestoreArrayF90(field%porosity0,porosity_loc_p,ierr)
    CHKERRQ(ierr)
    call VecRestoreArrayF90(clm_pf_idata%watsat_sub_pfp, porosity_loc_pfp, ierr)
    CHKERRQ(ierr)
    call VecRestoreArrayF90(clm_pf_idata%watpcwmax_sub_pfp, watpcwmax_loc_pfp, ierr)
    CHKERRQ(ierr)
    call VecRestoreArrayF90(clm_pf_idata%pcwmax_sub_pfp, pcwmax_loc_pfp, ierr)
    CHKERRQ(ierr)

    !
    call MappingSourceToDestination(pflotran_model%map_pf_sub_to_clm_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%watsat_sub_pfp, &
                                    clm_pf_idata%watsat_sub_clms)

    call MappingSourceToDestination(pflotran_model%map_pf_sub_to_clm_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%watpcwmax_sub_pfp, &
                                    clm_pf_idata%watpcwmax_sub_clms)

    call MappingSourceToDestination(pflotran_model%map_pf_sub_to_clm_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%pcwmax_sub_pfp, &
                                    clm_pf_idata%pcwmax_sub_clms)
...
!
  ...
  !!
  !
  subroutine pflotranModelSetGlobalTHfromCLM(pflotran_model, pf_hmode, pf_tmode)
...
#include "finclude/petscvec.h90"
    ...
    if (.not.pf_hmode) then
        call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%soillsat_sub_clmp, &
                                    clm_pf_idata%soillsat_sub_pfs)

        call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%soilpsi_sub_clmp, &
                                    clm_pf_idata%soilpsi_sub_pfs)

        call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%press_sub_clmp, &
                                    clm_pf_idata%press_sub_pfs)

        call VecGetArrayF90(clm_pf_idata%soillsat_sub_pfs, soillsat_pf_loc, ierr)
        CHKERRQ(ierr)
        call VecGetArrayF90(clm_pf_idata%press_sub_pfs, soilpress_pf_loc, ierr)
        CHKERRQ(ierr)
...
write(pflotran_model%option%myrank+200,*) 'checking pflotran-model 1 (CLM->PF lsat): ', &
        ...
        enddo
        call VecRestoreArrayF90(clm_pf_idata%soillsat_sub_pfs, soillsat_pf_loc, ierr)
        CHKERRQ(ierr)
        call VecRestoreArrayF90(clm_pf_idata%press_sub_pfs, soilpress_pf_loc, ierr)
        CHKERRQ(ierr)
    endif
    ...
    if (.not.pf_tmode) then
        call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%soilt_sub_clmp, &
                                    clm_pf_idata%soilt_sub_pfs)
        call VecGetArrayF90(clm_pf_idata%soilt_sub_pfs, soilt_pf_loc, ierr)
        CHKERRQ(ierr)
        ...
        enddo
        call VecRestoreArrayF90(clm_pf_idata%soilt_sub_pfs, soilt_pf_loc, ierr)
        CHKERRQ(ierr)
    endif

  end subroutine pflotranModelSetGlobalTHfromCLM
...
!
  ...
  ! author: Fengming YUAN
  ! date: 9/23/2013
  subroutine pflotranModelSetInternalTHfromCLM(pflotran_model)
...
end subroutine pflotranModelSetInternalThFromCLM


! ************************************************************************** !
!
  subroutine pflotranModelUpdateFlowCondsFromCLM(pflotran_model)
  !
  ...
  ! Date: 4/10/2013
  ! Updated: F-M Yuan, ORNL @ 03-14-2015
    ...
    ! Transpiration as Source/Sink term for 3D cells
    call pflotranModelUpdateTHSubSourceSink(pflotran_model)

    ! BC of flow mode
    if (pflotran_model%option%nsurfflowdof > 0) then
      call pflotranModelUpdateHSurfSource(pflotran_model)

    else
      ! mixed pressure-head (surface H2O) and ground-evap (dry) BC for
      ! subsurface 3-D domain (fmyuan)
      call pflotranModelSetSoilHbcsFromCLM(pflotran_model)
  ...
  end subroutine pflotranModelUpdateFlowCondsFromCLM
...
#include "finclude/petscvec.h90"
    ...
    call VecGetArrayF90(clm_pf_idata%area_subsurf_pfp, toparea_p, ierr)   ! needs further checking here (but should be OK either 's' or 'p')
    CHKERRQ(ierr)
    ...
    call VecRestoreArrayF90(clm_pf_idata%area_subsurf_pfp, toparea_p, ierr)
    CHKERRQ(ierr)
...
! ************************************************************************** !
!
  subroutine pflotranModelUpdateTHSubSourceSink(pflotran_model)
  !
  ! Update the source/sink term of 3D sub-domain
  !
  ! Author: Gautam Bisht
  ! Date: 11/22/2011
  !
...
#include "finclude/petscvec.h90"
    ...
    type(grid_type), pointer                  :: grid
    type(connection_set_type), pointer        :: cur_connection_set
    PetscScalar, pointer                      :: qflux_pf_loc(:)   ! water src/sink
    PetscScalar, pointer                      :: gflux_pf_loc(:)   ! heat src/sink
    PetscBool                                 :: found
    ...
    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%qflux_sub_clmp, &
                                    clm_pf_idata%qflux_sub_pfs)

    call MappingSourceToDestination(pflotran_model%map_clm_sub_to_pf_sub, &
                                    pflotran_model%option, &
                                    clm_pf_idata%gflux_sub_clmp, &
                                    clm_pf_idata%gflux_sub_pfs)
    ...
    ! Update the 'clm_transpiration_ss' source/sink term
    ! (TODO) heat src/sink needed for transpiration????

    call VecGetArrayF90(clm_pf_idata%qflux_sub_pfs, qflux_pf_loc,ierr)
    CHKERRQ(ierr)
    call VecGetArrayF90(clm_pf_idata%gflux_sub_pfs, gflux_pf_loc,ierr)
    CHKERRQ(ierr)
    ...
    do
      ...
      if(StringCompare(source_sink%name,'clm_transpiration_ss')) then

        found = PETSC_TRUE
        if (source_sink%flow_condition%rate%itype /= HET_MASS_RATE_SS) then
          call printErrMsg(pflotran_model%option,'clm_transpiration_ss is not of ' // &
                           'HET_MASS_RATE_SS')
        endif
        ...
        do iconn = 1, cur_connection_set%num_connections
          ...
          source_sink%flow_aux_real_var(press_dof,iconn) = qflux_pf_loc(ghosted_id)
    ...
    call VecRestoreArrayF90(clm_pf_idata%qflux_sub_pfs, qflux_pf_loc,ierr)
    CHKERRQ(ierr)
    call VecRestoreArrayF90(clm_pf_idata%gflux_sub_pfs, gflux_pf_loc,ierr)
    CHKERRQ(ierr)

    if(.not.found) &
      call printErrMsg(pflotran_model%option,'clm_tranpiration_ss not found in ' // &
                       'source-sink list of subsurface model.')

  end subroutine pflotranModelUpdateTHSubSourceSink


! ************************************************************************** !
!
  subroutine pflotranModelSurfHSource(pflotran_model)
  !
...
#include "finclude/petscvec.h90"
    ...
    type(connection_set_type), pointer        :: cur_connection_set
    class(surface_realization_type), pointer  :: surf_realization
    PetscScalar, pointer                      :: qh2o_srf_pf_loc(:)
    PetscBool                                 :: found
    PetscInt                                  :: iconn
    PetscErrorCode                            :: ierr
    ...
    call MappingSourceToDestination(pflotran_model%map_clm_srf_to_pf_srf, &
                                    pflotran_model%option, &
                                    clm_pf_idata%qh2o_srf_clmp, &
                                    clm_pf_idata%qh2o_srf_pfs)
    ...
    ! Source/sink terms -------------------------------------
    call VecGetArrayF90(clm_pf_idata%qh2o_srf_pfs, qh2o_srf_pf_loc,ierr)
    CHKERRQ(ierr)
    found = PETSC_FALSE
    ...
    do
      ...
      ! Find appropriate Source/Sink from the list of Source/Sinks
      if(StringCompare(source_sink%name,'srfh2o_rate_ss')) then

        found = PETSC_TRUE
        if (source_sink%flow_condition%rate%itype /= HET_VOL_RATE_SS) then
          call printErrMsg(pflotran_model%option,'srfh2o_rate_ss is not of ' // &
                           'HET_VOL_RATE_SS')
        endif

        do iconn = 1, cur_connection_set%num_connections
          source_sink%flow_aux_real_var(ONE_INTEGER,iconn) = qh2o_srf_pf_loc(iconn)
        enddo
      endif

      source_sink => source_sink%next
    enddo
    call VecRestoreArrayF90(clm_pf_idata%qh2o_srf_pfs, qh2o_srf_pf_loc,ierr)
    CHKERRQ(ierr)

    if(.not.found) &
      call printErrMsg(pflotran_model%option,'srfh2o_rate_ss not found in ' // &
                       'source-sink list of surface-flow model.')

  end subroutine pflotranModelSurfHSource
...
#include "finclude/petscvec.h90"
    ...
    if (clm_pf_idata%nlpf_subsurf <= 0 .and. clm_pf_idata%ngpf_subsurf <= 0 ) return
...
#include "finclude/petscvec.h90"
    ...
    do
      ...
      ! Find appropriate Source/Sink from the list of Source/Sinks
      if(StringCompare(source_sink%name,'clm_heat_srf_ss')) then

        found = PETSC_TRUE
        if (source_sink%flow_condition%energy_rate%itype /= HET_ENERGY_RATE_SS) then
          call printErrMsg(pflotran_model%option,'clm_heat_srf_ss is not of ' // &
                           'HET_ENERGY_RATE_SS')
        endif
    ...
    if (.not.found) &
      call printErrMsg(pflotran_model%option,'clm_heat_srf_ss not found in ' // &
...
! ************************************************************************** !
!
  subroutine pflotranModelGetHstatesFomPF(pflotran_model)
  !
  ! Extract soil saturation/pressure values simulated by
...
write(pflotran_model%option%myrank+200,*) 'checking pflotran-model 2 (PF->CLM lsat):  ', &
  ...
  end subroutine pflotranModelGetHstatesFromPF


! ************************************************************************** !
!
  subroutine pflotranModelGetTstatesFromPF(pflotran_model)
  !
...
#include "finclude/petscvec.h90"
  ...
  end subroutine pflotranModelGetTstatesFromPF



! ************************************************************************** !
!
  subroutine pflotranModelGetSurfFlowHeadFromPF(pflotran_model)
  !
...
#include "finclude/petscvec.h90"
  ...
  end subroutine pflotranModelGetSurfFlowHeadFromPF
...
!  end subroutine pflotranModelSetSOMKfromCLM
  ...
  ! ************************************************************************** !
  !
  ! pflotranModelSetBgcConcs:
  !  Get CLM concentrations (C, N), converts from CLM units into PFLOTRAN units.
  !  and set concentrations into PFLOTRAN
  ! ************************************************************************** !
  subroutine pflotranModelSetBgcConcsFromCLM(pflotran_model)
...
#include "finclude/petscvec.h90"
  ...
  !
  ! ************************************************************************** !
  subroutine pflotranModelUpdateAqConcsFromCLM(pflotran_model)
...
#include "finclude/petscvec.h90"
  ...
  end subroutine pflotranModelUpdateAqConcsFromCLM
...
! ************************************************************************** !
  ...
  subroutine pflotranModelGetBCMassBalDeltaFromPF(pflotran_model)
  !
...
!    endif
  ...
  end subroutine pflotranModelGetBCMassBalDeltaFromPF
